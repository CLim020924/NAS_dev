...
- 운영 완료: 경계 보완 commit `3bc145a`까지 NAS Linux에서 중간 symlink 공유 범위 이탈 검사를 포함한 Agent self-test와 backend 전체 14개 테스트 파일이 통과했고 문서 변환 통합 2건도 성공했다. PM2 restart/save 뒤 `msp-backend` online, `ssh`, `tailscaled`, `nginx`, `docker`, `pm2-root`, `cloudflared` active, 내부·공개 HTTP 200이다. 1.11.1 release commit의 GitHub/NAS 정합성과 위 해시는 다음 운영 배포 확인에서 최종 확정한다.
