...
- 현재 검증 경계: Windows backend 전체 테스트 131건 중 123 통과·8 환경상 skip·실패 0. 로그인된 실제 일반/관리자/마스터 화면의 첨부 저장→열기 클릭과 30일 시계 이동 통합 시험은 아직 하지 않았다. workbook 기존 한글 손상 때문에 관련 시트는 쓰지 않고 본 릴레이에 근거를 보존했다. 운영 배포 및 HTTP 검증은 이어서 수행한다.
