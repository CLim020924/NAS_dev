...
- 아직 완료로 표시하지 않는 항목: 운영 NAS 배포·공개 설치본 hash와 서비스/HTTP 검증은 이 commit 배포 단계에서 확인한다. 새 Windows 실제 CFAPI placeholder, 파일/폴더 선택 dialog와 모든 키 조합의 사람 손 상호작용은 별도 실PC E2E가 필요하다. `.msp-project` 완전 투영 marker가 없으므로 자동 popup은 계속 금지하고, 현재는 사용자가 제어센터에서 열고 F5로 재분석하는 방식만 제공한다.
