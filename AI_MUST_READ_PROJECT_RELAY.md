...
- 검증·배포: 새 helper, 기존 Note command와 clipboard focused Jest가 로컬·NAS에서 11/11 통과했고 양쪽 production build와 react-pdf 9.2.1/PDF.js API+Worker 4.8.69 gate가 통과했다. 운영 build와 `/var/www/html`은 `main.17763957.js` SHA-256이 일치한다. 내부 3030·공개 HTTPS 200, `ssh`·`tailscaled`·`nginx`·`docker`·`pm2-root`·`cloudflared` active, PM2 `msp-backend` online이다.
...
- 실제 NAS 발견·교정: 운영 NOTE MANAGER가 root:root 0700이므로 단순히 non-root 컨테이너로 낮추면 쓰기가 거부됐다. 컨테이너를 root로 올리지 않고, 실행 전 서버가 선택된 노트북 하나에만 전용 numeric UID 65532의 재귀 ACL을 부여하도록 수정했다. 계정 상위 폴더는 root 0700이고 다른 경로는 mount하지 않으므로 호스트·타계정 접근은 열리지 않는다.
- 운영 검증·배포: commits `34d616f`, `65cf9a9`를 GitHub와 NAS 활성 브랜치에 fast-forward했다. NAS 전체 backend 115개 중 110 pass·5 환경 skip·0 fail, 수정 후 focused 22/22와 root 운영 조건의 실제 Python·JavaScript·노트북 터미널 3/3이 통과했다. 터미널은 `/workspace/src`에 실제 파일을 만들고 읽었으며 `--network none` 연결 실패를 확인했다. production build/PDF.js API+Worker 4.8.69를 통과한 `main.318445b3.js`를 `/var/www/html`에 배포했다. `ssh`, `tailscaled`, `nginx`, `docker`, `pm2-root`, `cloudflared` active, PM2 `msp-backend` online/save, 내부 3030·공개 HTTPS 200, 무인증 files/terminal API 401, NAS checkout clean이다.
- 화면 검증 경계: 새 운영 bundle이 제공되는 공개 로그인 화면은 Chrome에서 직접 확인했다. 자동 브라우저에 인증된 NAS 세션이 없어서 실제 사용자 노트북 이름·파일을 열거나 자격정보를 재사용하지 않았으며, 노트북별 접기·옆 파일 목록 토글·명령 입력의 최종 육안 E2E는 로그인된 세션에서 한 번 확인해야 한다. 이는 구현·컨테이너·API·배포 검증과 분리해 기록한다.

## 2026-09-08 Note Studio 전체 사이드바·노트/프로젝트 구분·대화형 실행 콘솔

- 사용자 교정: “접기”는 각 노트북의 하위 목록만 접는 기능이 아니라 Note Studio의 왼쪽 목록 영역 전체를 사이드바처럼 숨기고 다시 여는 기능이다. 코드 실행은 결과 dialog가 아니라 편집기 하단의 숨길 수 있는 콘솔을 열고, 실행 중인 프로그램에 `input()`·Node `readline`과 같은 표준 입력을 전송해야 한다. 노트북은 Notion식 문서 중심 `노트`와 코드·파일·실행 중심 `프로젝트`를 생성 시 구분한다.
- UI 구현: 전체 왼쪽 사이드바를 헤더 버튼 또는 `Ctrl+B`로 접고, 접힌 동안 42px 복원 rail을 남긴다. 상태는 계정·장치별 Note Studio view-state에 저장한다. 실행 버튼은 기존 결과 dialog를 제거하고 하단 `실행 콘솔`을 연다. 콘솔은 stdout/stderr/system/input을 구분해 누적하고 자동 스크롤, stdin 입력, 중지, 기본/확대 높이, 숨기기와 재열기를 지원한다. 숨겨도 실행은 중단되지 않는다.
- 작업공간 모델: notebook registry에 `kind=notes|project`를 추가했다. 기존 kind 없는 노트북은 데이터 이동 없이 `notes`로 읽는다. 노트는 블록·Markdown·TXT 문서 흐름을 우선하고, 프로젝트는 실제 폴더·코드·터미널·실행 흐름을 우선한다. 둘 다 기존 NOTE MANAGER 물리 폴더와 페이지 계층을 공유하므로 별도 저장소나 계정 경계를 만들지 않는다. AI의 `create_notebook`도 kind를 명시해야 한다.
- 대화형 실행: 새 세션 manager와 4개 REST endpoint가 서버 저장본 revision·language·notebookId를 재검증하고, 선택한 물리 노트북 폴더 하나만 `/workspace`에 read-write mount한다. Docker stdin을 연 상태로 유지해 실행 중 입력을 전달하며 상태·증분 이벤트 cursor·중지·10분 결과 보존을 제공한다. 세션은 서버 인증에서 얻은 owner key에 묶여 다른 계정이 ID를 알아도 404로 거부한다.
- 자원·보안: 기존 resource admission을 세션이 끝날 때까지 점유하고 Python 512MiB/0.75 CPU, JavaScript 256MiB/0.5 CPU, PID 64, 최대 120초, 입력 합계 64KiB, 출력 256KiB를 제한한다. network none, non-root UID 65532, read-only rootfs, cap-drop ALL, no-new-privileges, 선택 노트북 ACL만 유지한다. 호스트 셸·다른 계정·Docker socket·인증정보는 전달하지 않는다. 일시적인 polling 오류는 프로그램 실패로 오판하지 않고 제한된 backoff로 자동 재연결한다.
...
### 2026-09-08 VS Code형 단일 하단 도킹 터미널 교정

- 사용자 교정: 단순히 터미널 높이를 줄이는 것이 아니라 VS Code처럼 노트·코드 편집기는 위에 계속 보이고 터미널은 같은 창의 맨 아래에만 도킹되어야 한다.
- 실제 원인: `terminalOpen` 조건이 참이면 메인 편집기 전체를 `NoteStudioTerminal`로 교체하는 삼항 렌더링 구조였다. 코드 실행 콘솔만 편집기 아래 형제였기 때문에 일반 터미널과 실행 콘솔의 배치 규칙도 서로 달랐다.
- 구현: 메인 편집기/프로젝트 안내 렌더링과 하단 패널 렌더링을 완전히 분리했다. 일반 터미널은 180~320px, 작업영역 최대 42%의 하단 dock으로 표시한다. 일반 터미널과 코드 실행 콘솔은 같은 하단 자리에서 하나만 보이며, 코드 실행은 일반 터미널을 닫고 실행 콘솔을 우선하고 터미널 열기는 실행 콘솔을 숨긴다. 숨겨진 실행 콘솔 재열기 버튼도 터미널을 닫고 즉시 실행 결과를 표시한다.
...
### 2026-09-08 Note Studio 전체 등록 언어·실행 대기열

- 사용자 요청: 현재 편집기에 등록된 언어를 자동 감지하고 자동완성·편집 기능과 실제 동작을 연결한다. 여러 사용자가 동시에 또는 연달아 실행하면 실시간 서버 자원과 사용자 제한을 계산해 안전한 요청은 즉시 실행하고 나머지는 순서대로 대기시킨다.
- 정확한 동작 구분: `plaintext`는 편집 전용, Python·JavaScript·TypeScript·Shell·SQL은 실행, JSON·YAML은 문법 검증과 정리 출력, HTML·CSS·Markdown은 미리보기다. 모든 항목을 컴파일이라고 부르지 않는다. Monaco 언어 서비스가 구문 강조·기본 완성을 제공하고 import는 확장자 우선, 새 plaintext 코드는 shebang·보수적 내용 힌트로 자동 감지하며 사용자의 수동 선택을 덮지 않는다.
- 격리 실행: 기존 Python/JavaScript 세션 manager에 TypeScript(Node 22 type stripping), Shell, 메모리 SQLite, 신뢰된 JSON/YAML validator를 추가했다. network none, non-root UID 65532, read-only rootfs, cap-drop ALL, no-new-privileges, CPU/RAM/PID/120초/입출력 제한과 현재 계정의 선택 노트북 하나만 read-write mount하는 경계는 동일하다. HTML/CSS는 sandbox iframe과 `default-src 'none'` CSP, Markdown은 raw HTML을 실행하지 않는 GFM renderer로 서버 실행과 분리했다.
- 실제 queue: resourceControl의 `queued`를 429로 폐기하던 경로를 owner-bound 실행 작업으로 바꿨다. 사용자 교대 우선 FIFO로 한 사용자의 burst가 다른 사용자를 계속 밀지 않게 하며 순번·이유·시작 전환을 하단 콘솔에 표시하고 대기 취소를 지원한다. 기존 실행 때문에 생긴 사용자 동시성/합산 한도는 대기하고, 요청 하나 자체가 정책을 넘거나 서버 hard gate에 걸리면 차단한다. 종료·실패·취소에서 예약을 회수하고 작업 메타는 10분 뒤 정리한다.
- 발견·교정: NAS Docker smoke에서 한 줄에 여러 SQL 문장이 있으면 Python sqlite API가 한 번에 한 문장만 허용해 실패했다. trusted runner가 문자 단위 `sqlite3.complete_statement`로 문장 경계를 분리하도록 고쳐 create/insert/select 연속 실행을 통과했다.
- 검증·배포: 코드 commits `7f321a3`, `d34d683`, verifier `349c7d9`를 GitHub와 NAS 활성 브랜치에 fast-forward했다. 로컬 focused backend 12/12와 production/PDF.js 4.8.69 build를 통과했다. NAS 전체 backend는 122개 중 116 pass·6 환경 skip·0 fail, 실행 콘솔 2/2, UI contract와 compact verifier, 실제 Docker Python/JavaScript/TypeScript/Shell/JSON/YAML/SQL 7/7을 통과했다. 운영 `main.140f9a32.js`는 로컬 build와 SHA-256이 일치하고 내부 3030·공개 HTTPS 200, 무인증 language/queue API 401, 필수 서비스 active, PM2 `msp-backend` online/save다.
- 기록·남은 경계: `NAS_PROJECT_LOG.xlsx`와 `NAS_NOTE_STUDIO_SPEC.xlsx`에 기능·관계·코드·API·보안·상태기계·성능·테스트·변경 상태를 갱신했다. artifact-tool로 사전/사후 렌더, 재열기와 수식 오류 검사를 완료했다. 로그인된 실제 화면에서 자동 감지·HTML/CSS/Markdown 미리보기와 두 계정 동시 queue 순번/취소를 누르는 최종 E2E는 인증 세션에서 남는다. 현재 TypeScript는 단일 파일의 erasable syntax 범위이며 TSX/JSX 다중 파일 bundling, C/C++/Java compiler, 서버 재시작 뒤에도 남는 durable multi-worker build queue는 별도 단계다.

### 2026-09-08 VS Code Explorer 조사 기반 노트북 메뉴·개발 도구

- 사용자 요청과 교정: 구현부터 시작하지 말고 VS Code에서 노트북/파일 트리 우클릭 메뉴와 확장 검색이 실제로 어떻게 배치되고 동작하는지 정확히 조사한 뒤 NAS 프로젝트에 맞게 구현한다.
- 조사 결과: VS Code Explorer는 고정 메뉴가 아니라 대상의 `contextValue`, 선택 상태, 파일/폴더 여부와 권한을 `when` 조건으로 평가해 명령을 노출한다. 중요한 인라인 동작은 최대 3개로 제한하고 나머지는 우클릭과 submenu에 둔다. 확장은 UI·workspace·web Extension Host에서 실행되며 Monaco 편집기만으로 임의 VSIX를 실행할 수 없다. 관련 공식 근거와 실제 Explorer 명령 그룹은 `docs/NOTE_STUDIO_VSCODE_EXPLORER_EXTENSION_RESEARCH.md`에 기록했다.
- 구현: 노트북 행 인라인에는 `새 페이지`와 `더보기`만 두고, 우클릭에는 선택, 접기/펼치기, 새 페이지, 터미널, 프로젝트 전용 개발 도구, NAS 파일 관리자에서 열기, 표시 이름 변경(F2), NAS 경로 복사를 대상 상태에 맞게 배치했다. 표시 이름 변경은 동기화·코드 참조를 깨지 않도록 물리 디렉터리 이름과 경로를 유지하고 registry의 title/revision만 갱신한다. 오래된 revision 없는 노트북은 revision 1로 안전하게 읽는다.
- 확장 연결: 서버의 고정 Open VSX origin을 통한 검색·상세 API와 5분 캐시를 추가했다. 프로젝트형 노트북에서만 선택한 `publisher.extension`을 실제 프로젝트 폴더의 `.vscode/extensions.json` recommendations에 중복 없이 추가·제거한다. 기존 `unwantedRecommendations`와 알 수 없는 필드는 보존하고 symlink·잘못된 ID·손상된 JSON은 거부한다. NAS에서 VSIX를 다운로드·자동 설치·실행하지 않으며 web compatible 결과도 `adapter-required`로 표시한다.
...
### 2026-09-20 소수 사용자 링크 배포 시 승인 조건

- 사용자 질문: 본인만 또는 링크를 받은 소수만 쓰는 모바일 공유 백업 앱도 플랫폼 인증·심사가 복잡한가.
- iOS 공식 배포 조건: 무료 Apple Account/Xcode는 본인 기기 개발 테스트용이고 주기적 재프로비저닝 제한이 있다. 다른 사용자에게 TestFlight 링크를 제공하려면 유료 Apple Developer Program과 외부 첫 빌드 TestFlight 검토가 필요하며 베타 빌드는 90일 후 만료된다. App Store의 미등록(unlisted) 앱은 직접 링크로만 발견되지만 정식 App Review와 별도 미등록 배포 신청이 필요하고, 링크 보유자는 누구나 설치 가능하므로 NAS 로그인/초대 승인으로 사용 권한을 제한해야 한다. Ad Hoc 기기 지정 배포도 유료 멤버십 영역이다.
- Android 공식 소수 배포: 스토어 밖 APK 직접 배포가 가능하나 설치 출처 허용 및 기기 정책 확인이 필요하다. 2026년 개설된 무료 Android Developer Console 제한 배포 계정은 비상업적 취미/학습용 앱을 최대 20대에 QR/링크·사용자 동의 방식으로 등록할 수 있고 정부 신분증 제출은 요구하지 않는다. 향후 Android 개발자 검증 확대를 고려하면 장기 소수 배포 후보로 검토한다. 사용 목적이 이 계정 자격에 맞는지는 실제 신청 시 확인한다.
- 보안·다음 단계: 배포 링크의 비공개성과 NAS 자료 접근 권한은 별개다. 앱 설치 뒤 NAS 계정 인증·초대/허용 목록·기기/토큰 취소·계정 root/쿼터 검증이 필요하다. 이번 요청은 정책 조사만 수행했으며 앱 구현·심사 신청·실기기 테스트는 하지 않았다.

### 2026-09-20 Apple 개발자 가입 비용 질문

- 사용자 질문: 소수 사용자 iPhone 앱 배포에 필요한 Apple Developer Program 가입비가 비싼가.
- Apple 공식 안내 재확인: 연 US$99 또는 등록 지역의 현지 통화 가격이며, 세금/결제 최종액은 가입 화면에서 확인한다. 본인 기기 개발 테스트는 무료 Apple Account로 가능하다. 무료 계정은 다른 사용자에게 안정적으로 앱을 배포하는 수단이 아니며, TestFlight/App Store/Ad Hoc 소수 배포는 유료 멤버십이 필요하다.
- 이번 요청은 가격 질문에 대한 답변만 수행했다. 가입·결제·앱 구현은 하지 않았다.

### 2026-09-20 유료 Apple 개발자 가입 없이 iPhone 공유 메뉴 사용

- 사용자 정정: 유료 가입을 하지 않을 경우 본인만 사용할 수 있는지 묻는다. 앞선 답변은 무료 네이티브 앱 개발 테스트와 무료 대체 수신 경로를 구분하지 못했다.
- Apple 공식 문서 확인: 무료 Personal Team의 iOS 네이티브 앱은 본인 기기 개발 테스트에 초점을 두며 프로비저닝은 7일 후 만료된다. 타인에게 일반 앱으로 안정적 배포하려면 유료 개발자 프로그램이 필요하다.
...
### 2026-09-20 루트 정리 후 채팅·받은 파일·보관 정책 교차 점검

- 사용자 요청: 이전에 설정한 채팅, `받은 파일`, 삭제 처리 기간 등이 계정 경로 정리로 틀어졌는지 조사하고 수정한다.
- 실제 결함: 채팅 저장은 MASTER/MANAGER의 개인 `/users/<id>/받은 파일`로 향하는데, 채팅 `viewerOpenTarget`과 저장 이력의 요청 경로는 이전처럼 `/받은 파일`이었다. 특권 계정의 파일 관리자에서는 `/`가 NAS 전체 루트라 저장 후 열기가 다른 경로를 가리킨다. 일반 계정은 개인 루트 기준 `/받은 파일`이 맞다. 첨부 임시 업로드의 quota 검사에는 NAS 루트의 임시 경로가 전달되어 특권 계정 개인 할당량을 우회할 수 있었고, 받은 파일로 복사할 때는 수신자 quota 검사가 없었다.
- 수정: 저장 물리 경로와 UI 요청 경로를 권한별 access root에서 계산하는 공통 helper를 추가했다. 일반 계정은 `/받은 파일`, MASTER/MANAGER는 `/users/<id>/받은 파일`을 반환하고 저장 이력 조회도 동일한 access root로 확인한다. 채팅 임시 첨부 생성과 수신자 저장 전에 개인 할당량/물리 여유를 검사하며 복사 전후 사용량 캐시를 무효화한다. 보관 기간 값 자체는 임의로 바꾸지 않았다.
- 운영 읽기 검증: `messages.json` 123건 중 30일 초과 0건, 첨부 묶음 2건 중 만료 0건, 실제 수신 파일 1건은 약 19일 경과했다. 이전 루트에서 보존 폴더로 옮긴 `받은 파일` 디렉터리는 모두 비어 있었다. `msp-backend`의 최근 retention 로그는 startup 성공, archived/removed 0을 보고했다. 수신 파일 30일, 발신 임시 첨부 30일, pending 첨부 24시간, 메시지 30일 후 `chatdata` 아카이브 정책은 기존 그대로다.
- 현재 검증 경계: Windows backend 전체 테스트 131건 중 123 통과·8 환경상 skip·실패 0. 로그인된 실제 일반/관리자/마스터 화면의 첨부 저장→열기 클릭과 30일 시계 이동 통합 시험은 아직 하지 않았다. workbook 기존 한글 손상 때문에 관련 시트는 쓰지 않고 본 릴레이에 근거를 보존했다.
- 운영 적용: `939069c`를 GitHub와 NAS 활성 브랜치에 fast-forward하고 Linux 경로·quota 회귀 테스트 9/9 통과 후 root PM2 백엔드를 재시작·저장했다. 내부 `127.0.0.1:3030`과 공개 사이트가 HTTP 200, NAS Git 작업 트리 clean이다. 재시작 뒤 retention은 메시지 123건·첨부 묶음 2건 유지, 삭제/아카이브 0건으로 성공했다. 이전 루트 계정들의 보존된 `받은 파일` 디렉터리도 모두 비어 있어 보관 대상이 빠진 사례는 확인되지 않았다. 실제 계정 UI E2E는 여전히 확인 필요하다.

### 2026-09-20 채팅 저장·보관 전체 재검증 진행

- 사용자 요청: 앞선 계정/채팅 경로 수정이 실제 사용자 흐름과 예전 보관 정책 전체에서 맞는지 끝까지 시험하고 결함은 해결한다.
- 실사용 UI는 Chrome의 공개 사이트를 열었으나 인증 세션이 없어 로그인 화면에서 대기 중이다. 비밀번호를 자동 입력하거나 기존 계정에게 시험 메시지를 보내지 않았다. 사용자가 브라우저에 직접 로그인하면 일반/특권 계정의 첨부 저장·열기·재다운로드 흐름을 육안 확인할 수 있다.
- 격리 코드 시험에서 확인한 결함: 같은 메시지의 `받은 파일에 저장` 재시도는 기존 파일을 중복 복사해 용량을 소비했다. 일부만 사라져 재다운로드하는 경우에도 전부 다시 복사했다. 24시간 pending/canceled 첨부 묶음 정리는 새 첨부 요청이 있어야 실행되어 서버 유휴 시 방치됐다. 만료 파일 삭제 실패에도 첨부 메타데이터는 제거될 수 있었고, 메시지 아카이브 후 live JSON 정리 전 장애가 나면 재시도 시 중복 JSONL 기록이 생길 수 있었다.
- 수정·테스트: 수신 파일 복사를 별도 helper로 분리해 저장 이력이 유효하면 중복 복사를 거부하고, 일부만 누락되면 필요한 원본만 재복사·quota 계산한다. 원본/용량 검사 실패 시 복사하지 않으며 중간 복사 실패의 새 결과물은 되돌린다. 서버 시작·매시간 retention에 pending 묶음 정리를 포함하고, 24시간/30일 만료 기준과 실패 시 기록 보존을 확인한다. 메시지 archive ID를 월별 파일에서 확인해 장애 재시도 시 중복 기록을 막는다. 운영 파일 대신 OS 임시 디렉터리의 테스트용 JSON·파일로 정상/만료/재시도/충돌을 시험했다. 현재 로컬 격리 시험 9건 통과, 운영 적용 전이다.
- 운영 read-only 사전 확인: 실제 `chatAttachments.json` 2건은 모두 sent이고 24시간 초과 pending 묶음은 0건이다. 정책 수치(메시지·발신 첨부·수신 파일 30일, pending 24시간)를 바꾸지 않았다. 기존 workbook은 한글 문자열이 깨진 상태라 추가 편집을 보류하고 이 릴레이에 정확히 기록한다.
- 운영 적용·재검증: 수정 커밋 `16f7654`를 원격과 NAS 활성 브랜치에 fast-forward했다. NAS Linux의 새 경로/보관 테스트 12/12 통과, Windows backend 전체 140건 중 132 통과·8 환경상 skip·실패 0이다. 운영 데이터를 재확인해 메시지 123건, sent 첨부 묶음 2건, 만료 pending 0건인 상태에서 root PM2 `msp-backend`를 재시작·저장했다. 재시작 후 PM2 online/unstable restart 0, 내부 HTTP 200, 공개 HTTPS 200, startup retention의 삭제 0건·메시지 123건·첨부 2건 유지·pending 삭제 0건을 확인했다. PM2 error.log의 JSON 파싱/중단된 multipart 오류는 마지막 수정 시각이 이번 재시작 전인 2026-09-19 22:01 KST이므로 이번 배포의 새 오류는 아니다.
- 남은 실제 화면 검증: 공개 Chrome 탭이 `/login`이고 사용자의 로그인 응답이 아직 없어 실계정 첨부 저장→열기와 재다운로드, 일반/관리자/마스터 화면별 경로 확인은 완료했다고 주장하지 않는다. 운영 계정에 시험 메시지를 발송하지 않았고 운영 보관 시계를 변경하지 않았다. 사용자가 직접 브라우저 로그인 후 읽기 위주 UI 검증을 재개한다. `docs/NAS_PROJECT_LOG.xlsx`는 기존 한글 손상 복원 전 편집 보류다.

### 2026-09-20 웹 동시 접속·로그아웃 동작 진단

- 사용자 요청: 현재 동시 접속과 로그아웃 동작을 살펴보고 자체적으로 오류를 찾아 보완 필요 지점을 보고한다. 이번 턴은 진단 요청으로 해석해 운영 세션 강제 종료나 인증 코드 변경은 하지 않았다.
- 확인된 동작: 정상 비밀번호 로그인에서 기존 세션이 있으면 409 선택창을 내며 `allow`는 세션 추가, `replace`는 기존 세션 기록 초기화와 해당 연결 소켓 종료를 수행한다. 일반 로그아웃은 해당 sessionId만 목록에서 제거하고 쿠키를 지운다. 세션 유지 선택은 사용자별 설정이며 일반 토큰 1일/유지 토큰 30일이다. 일반 세션은 마지막 소켓 연결이 끊어진 뒤 기본 60초가 지나면 목록에서 제거된다.
- 코드상 결함/위험: 일반 로그아웃은 해당 인증 소켓을 서버에서 직접 끊지 않는다. 따라서 클라이언트가 소켓을 닫기 전 기존 회의·커서·아이콘 이벤트 핸들러가 살아 있을 수 있고, 관리자 온라인 표시는 연결 소켓을 기준으로 계산된다. 로그인 충돌 검사는 `activeSessions`를 그대로 세며 만료 JWT에 해당하는 항목을 주기적으로 제거하지 않는다. 소켓 없이 발급된 세션이나 영속 세션은 기록이 남아 재로그인 때 잘못된 충돌 창을 유발할 수 있다. 반대로 비영속 세션은 네트워크 단절 60초 후 제거되어 아직 만료되지 않은 브라우저 토큰도 무효가 될 수 있다. 로그아웃 API 실패 시 프런트는 로컬 사용자만 지우고 쿠키·서버 세션은 남길 수 있다. 인증/동시접속 회귀 테스트는 현재 별도로 확인되지 않았다.
- 운영 읽기 전용 집계: 24계정에 activeSessions 18개, 복수 세션 계정 5개, 발급 시각이 명목 토큰 기간(비영속 1일/영속 30일)을 넘은 기록 8개다. 토큰 갱신 시 발급 기록의 갱신 여부는 확인하지 않았으므로 이 8개를 모두 실제 무효 세션으로 단정하지 않는다. 세션 ID·토큰·계정명은 출력하거나 기록하지 않았다.
- 다음 안전 작업: 세션 수명/충돌 판단을 별도 모듈과 격리 테스트로 검증하고, 로그아웃·교체·계정 비활성화 시 해당 소켓 즉시 차단, 다중 탭/단절/재연결 정책을 명시한 뒤 실제 두 브라우저·두 기기에서 E2E 확인한다. 공개 사이트 브라우저는 직전 확인 시 로그인 화면이라 실제 계정 UI 조작은 아직 미검증이다. 기존 workbook의 한국어 손상이 있어 관련 시트 수정은 복원 후 보류한다.

### 2026-09-20 전원 종료 후 다른 장소 로그인 충돌 보고

- 사용자 보고: 이전 PC에서 로그아웃하지 않고 전원을 끈 뒤 다른 장소에서 같은 계정으로 접속하면 동시 로그인 여부를 묻는 현상이 가장 잦았다. 이는 사용자의 체감 보고이며 이번 턴에 실제 두 PC로 재현한 결과는 아니다.
- 코드 대조: `/api/login`은 `activeSessions.length > 0`만으로 충돌을 결정하며 현재 소켓 접속/장치 전원 상태는 검사하지 않는다. `loginPersistenceEnabled`가 켜진 세션은 소켓 disconnect에서 제거하지 않는다. 꺼진 PC의 영속 세션이 서버 장부에 남으면 다른 장소에서 즉시 409 선택창이 뜬다. 유지가 꺼진 세션도 연결 종료 감지 후 기본 60초의 정리 유예와 감지 지연 동안 동일 창이 뜰 수 있다. 이는 코드상 재현 가능한 원인 설명이며 실제 사용자 당시 설정은 아직 미확인이다.
- 정책 구분: 전원 종료는 로그아웃과 동일하지 않다. 유지 세션의 토큰은 PC 재부팅 후에도 유효할 수 있으므로 무조건 삭제하거나 다른 기기의 접속으로 조용히 교체하면 안 된다. 다만 현재 UI의 `이미 로그인`은 `지금 온라인`과 `오프라인이지만 유효한 로그인 기록`을 구분하지 않아 오해를 만든다.
- 다음 작업: 서버가 실제 연결 중인 세션과 오프라인 유효 세션을 분리 표시하고, 만료/유실 기록을 정리하는 정책을 설계한다. `동시 접속 허용`, `기존 세션 종료`, `취소`의 영향 범위를 명시한 뒤 전원 끄기/강제 종료/브라우저 닫기/일시적 네트워크 단절/재부팅을 두 기기에서 검증한다. 이번 턴은 진단과 기록만 했으며 인증·운영 세션은 변경하지 않았다.

### 2026-09-20 오프라인 로그인 기록 구분 및 마지막 작업 복구

- 사용자 요청: 전원 종료 후 다른 장소에서 같은 계정으로 로그인할 때 나타나는 동시 접속 안내를 해결하면서, 파일/페이지 등 마지막 작업으로 돌아가는 버튼을 새 로그인 직후에만 제공한다. 다른 파일이나 작업을 열면 버튼은 사라져야 한다. 사용자 선택으로 꺼진 PC의 유효 로그인 기록은 실제 온라인 접속과 구분해 유지/종료를 직접 선택하게 한다.
- 인증 변경: 로그인 충돌 판단 전에 만료 세션 기록을 정리하고 서버에 연결된 Socket.IO session ID로 온라인/오프라인 유효 세션 수를 구분한다. 새 세션과 유지 설정 갱신 시 토큰 만료 시각을 세션 기록에도 넣는다. 일반 로그아웃은 해당 session ID의 소켓을 서버에서 즉시 끊는다. 새 로그인 선택창은 오프라인 기록의 의미를 설명하고 기존 기록 유지/종료/취소를 제공한다. 전원 종료만으로 유효한 영속 로그인 토큰을 임의 폐기하지 않는다.
- 복구 변경: 계정별 기존 `workspace-session` view-state 원장에 마지막 파일/폴더/앱 포인터만 저장한다. 새 로그인 시 해당 계정의 이전 작업이 여전히 접근 가능할 때만 상단 아래에 복구 버튼을 제안하고, 파일·폴더·노트/문서 앱·채팅/AI 등 다른 작업을 열면 제안을 닫는다. 버튼을 누르면 대상 파일/앱을 열고 기존 PDF 페이지/줌, 코드 커서, 노트 페이지/블록 등 개별 view-state 복원을 재사용한다. 타 계정 경로, 삭제된 항목, 지원하지 않는 앱은 제안하지 않는다. OnlyOffice 내부 문자 커서처럼 기존에 캡처하지 못하던 위치를 새로 지원한다고 주장하지 않는다.
- 로컬 검증: Windows backend 전체 144건 중 136 pass·8 환경 skip·0 fail, 신규 세션 분류 5건 및 복구 제안 2건 통과. 프런트 production compile, PDF.js API/Worker gate, window manager verifier, backend 구문/whitespace 검사가 통과했다. Windows Jest 기본 jsdom은 기존 native `canvas.node` 부재로 실행되지 않아 신규 순수 테스트에 Node 환경을 지정해 통과했다. 프런트 빌드는 기존 중복 ESLint 플러그인 충돌을 피해 ESLint 플러그인만 끄고 compile했다.
- 운영 적용: `8be7320`을 활성 브랜치에 push하고 NAS 작업 트리에 fast-forward했다. 운영 인증 원장 `backend/data/members.json`을 내용 노출 없이 권한 600의 별도 백업으로 보존한 후, Windows에서 빌드한 프런트를 NAS 임시 스테이징에 전송하고 `/var/www/html`에 index 마지막 순서로 반영했다. 배포 파일 SHA-256을 스테이징/운영 위치에서 대조했으며 root PM2 `msp-backend`를 재시작·저장했다. NAS 자체 신규 테스트 5건/구문 검사 통과, backend/public/bundle HTTP 200, 무인증 view-state 401, PM2 online·재시작 횟수 3을 확인했다. 이미 로그인된 Chrome 탭을 새로고침해 NAS 파일 관리자 표시 및 브라우저 오류 로그 0건을 확인했다.
- 미검증 경계: 실제 서로 다른 두 PC의 전원 종료·재부팅·동시 로그인 선택과 새 로그인 직후 복구 버튼 클릭/다른 파일 열기 자동 소멸은 아직 실계정 E2E로 확인하지 못했다. 기존 로그인 탭의 새로고침은 이 흐름을 대체하지 않는다. 기존 `docs/NAS_PROJECT_LOG.xlsx`는 이미 한국어 문자열/시트명이 손상되어 있으므로 복원 전 추가 편집을 보류하고 안전한 릴레이에 변경 경계를 기록한다.

### 2026-09-20 NAS Drive 계정 제거 즉시 반영 및 PDF 표시 반복 수정

- 요청: 드라이버에서 제거한 계정이 Windows 파일관리자 좌측 NAS 목록에서 곧바로 사라지게 하고, 전체 상태 표시·UI 반응·PDF 저장 상태를 불러올 때의 반복적인 새로 그리기/지연을 조사·개선한다.
- 확인 원인: 드라이버 로그아웃은 로컬 설정과 토큰을 먼저 지운 후 서버 revoke 응답을 최대 15초 기다리고 탐색기 Home pin/CFAPI SyncRoot를 해제했다. 서버가 느리거나 오프라인이면 제거된 계정 항목이 남았다. 현재 PC는 로컬 config에 profile 1개인데 현재 사용자 NAS SyncRoot가 2개였고, 남은 등록 하나는 config에 없는 계정이며 그 Provider 프로세스가 없었다. PDF는 주석 GET 성공 시 `markDirty(false)`가 부모 창 배열을 매번 새 객체로 만들고 부모의 `onDirtyChange` 함수도 렌더마다 바뀌어 주석 GET effect가 재실행될 수 있는 순환이 있었다.
- 수정: Agent 1.11.5는 로컬 등록/핀을 먼저 해제하고 Shell 부모 폴더·아이콘 갱신을 알린 후 나머지 계정 background를 복구한다. 서버 revoke는 그 뒤에 시도하며 네트워크 실패가 로컬 목록 제거를 막지 않는다. unregister 실패는 로그에 남긴다. Native 제어창은 config 제거만으로 완료를 표시하지 않고 SyncRoot 등록 소멸을 확인한다. Agent가 비정상 종료되면 긴급 로컬 로그아웃이 현재 계정의 config/선택된 Drive/등록된 root에서 정확한 대상 경로를 얻어 Home pin과 SyncRoot를 해제한다. 원본 로컬 폴더는 삭제하지 않는다. PDF 쪽은 동일한 dirty 값에는 창 배열을 그대로 유지하고 콜백을 안정화해 주석 불러오기 effect의 재시작 경로를 없앤다. Windows 설치 파일과 Agent 다운로드 버전을 1.11.5로 맞춘다.
- 현재 PC 보정: config에 없는 오래된 NAS SyncRoot 하나만 정확히 확인해 기존 Provider `unregister`로 해제했다. 종료 코드 0, NAS 등록 수 2→1, 해당 로컬 폴더는 그대로 보존됨을 확인했다. 현재 활성 profile/Provider는 변경하지 않았다.
- 검증/배포 경계: 신규 UI 무변경 상태 테스트, Agent/Setup 자체 테스트, deviceSyncSecurity 회귀, backend 전체 145건(137 pass·8 환경 skip), 프런트 production build와 PDF.js API/Worker gate가 통과했다. 실계정 계정 제거 클릭·Explorer 화면 즉시 소멸·실제 PDF 파일 장시간 렌더/네트워크 요청 횟수 비교는 아직 수행 전이며 현재 근거만으로 완전 검증을 주장하지 않는다. 기존 workbook의 한국어 문자열 및 `보류 작업` 시트명 손상이 다시 확인되어 복원 전까지 xlsx 자체 편집은 보류한다.
- 운영 반영/실화면 확인: `6451c9e`를 활성 원격 브랜치에 push하고 NAS 작업 트리를 fast-forward했다. 이전 웹 index와 Agent/Setup 바이너리는 권한 제한된 runtime-backups 하위에 보존했다. 프런트 빌드를 staging에서 웹 루트로 반영하고 index를 마지막에 교체했으며, 새 JS 번들의 staging/운영 SHA-256이 일치한다. PM2 `msp-backend` 재시작·저장 후 online, backend 루트/public NAS/새 bundle HTTP 200 확인. 기존 `/api/health`는 없는 경로라 404이며 장애 증거로 사용하지 않는다. Chrome에서 실제 `/users/dntdlzz/루트에서 정리됨/2026-09-20/머신러닝/Ch12_비지도학습.pdf`를 열어 33페이지 렌더, `저장됨` 상태, 재열기 시 `pdf-annotations` GET 1회, 브라우저 오류 로그 0건, 첫 페이지 시각 표시를 확인했다. 기존 최근 파일 항목은 이동 전 경로(`/루트에서 정리됨/...`)를 가리켜 파일 없음으로 열렸으므로 최근 파일 경로 갱신은 별도 후속 과제다.
- 잔여 검증/성능 경계: 현재 PC의 오래된 탐색기 등록은 2→1로 정리했으나 실제 다중 계정의 제거 클릭 직후 Explorer 좌측 항목 소멸까지 자동 UI 검증하지는 못했다. 이 PC에 설치된 Native Launcher는 아직 1.11.4이므로 새 1.11.5 Setup 실행 후 긴급 로그아웃 경로 검증이 필요하다. PDF는 33페이지를 동시에 렌더하는 구조여서 대형 문서에서 발생할 수 있는 렌더 지연·메모리 부담은 별도 가상화/성능 측정이 필요하다. 실제 사용자 체감 지연이 전혀 없다고 단정하지 않는다.

### 2026-09-20 로그인 실시간 상태 및 계정 경로 접근 기록 요청

- 요청: 모든 사용자의 실제 로그인 온라인/오프라인을 브라우저 종료 후 빠르게 반영하고 메모리·프로세스 효율을 유지한다. 그다음 타인이 계정 경로에 접근·변경·다운로드한 기록과 알림, 상단 접근 기록 UI, 타인 변경의 15일 복원, 드라이브 실시간 동기화까지 구현한다.
- 실시간 상태 원인/1차 수정: 인증 소켓 자체는 사용자별 온라인 판정에 쓰였으나 친구 목록은 열려 있는 동안 5초마다 전체 응답을 재조회했고 관리자 사용자 화면의 5초 반복 조회는 `users` 상태를 갱신하지 않았다. Socket.IO `membersChanged`를 최초 접속/마지막 종료에 바로 방송하고, 다중 탭 중 하나만 닫아도 온라인을 유지한다. 친구 화면은 이벤트 기반/30초 복구 폴링 및 오래된 응답 차단, 관리자 화면은 경량 `/api/users/presence`와 30초 전체 갱신으로 수정했다. 소켓 ping 10초/timeout 8초로 비정상 단절 감지 상한을 줄였다. 정상 브라우저 종료는 disconnect 이벤트 뒤 즉시 반영되지만 전원·망 장애는 최대 약 18초 감지 지연이 가능하며 물리적으로 0초 보장은 불가능하다.
- 현재 검증: 신규 다중 탭·오프라인 기록 테스트와 backend 전체 146건(138 pass, 8 환경 skip, 0 fail), 프런트 production build 성공. 실제 두 기기 종료·전원 단절 E2E와 운영 배포 후 사용자 화면 반영 시간은 후속 검증 필요.
- 운영 화면 확인 중 추가 수정: 관리자 `사용자 관리`의 사용자 표에는 `isOnline` 값이 와도 시각 표시가 전혀 없었다. 모바일 카드와 데스크톱 표의 이름 옆에 텍스트가 있는 온라인/오프라인 칩을 추가했다. 공개 운영 브라우저에서 이 누락을 재현했고 수정 후 프런트 production build가 다시 통과했다.
- 접근 기록 설계상 위험: 기존 `/activity`와 파일 버전은 접근 base 경로 단위이며 마스터/관리자가 다른 계정의 `/users/<계정>`에 접근해도 소유자별 기록이 자동 생성되지 않는다. 공개 공유 링크는 로그인 없는 다운로드를 허용하므로 다운로드한 실제 인물을 현재 구조만으로 식별할 수 없다. 로그인 강제 적용 범위는 사용자 선택 대기 중이다. 기존 30일 휴지통/버전 기록을 외부인 변경에 대한 정확한 15일 복원과 혼동하지 않는다. 변경 경로·파일별 권한/쿼터·상충 변경·동기화 재전파까지 검증하기 전 접근 기록/복원 UI를 완료로 표시하지 않는다.
- 워크북: canonical `docs/NAS_PROJECT_LOG.xlsx`의 시트 이름과 한국어 셀이 기존부터 치환 문자로 손상되어 있어 원본을 덮어쓰지 않았다. 이 항목은 릴레이에 기록하며 복구 가능한 원본 확보 후 Request_Archive/Patch_Log/Feature_Index/Relation_Map/보류 작업에 반영한다.
- 실시간 상태 운영 검증: `5fbbabe`와 `5e839a9`를 원격/NAS 활성 브랜치에 fast-forward했다. 이전 index를 제한된 backup에 남기고 새 빌드의 staging/운영 해시 일치, PM2 online/save, backend/public HTTP 200, 무인증 `/api/users/presence` 403을 확인했다. 공개 Chrome 로그인 세션의 관리자 `사용자 관리`에서 자기 계정이 `온라인`, 로그아웃 상태인 다른 계정은 `오프라인`으로 표시됨을 확인했다. 같은 계정의 두 번째 브라우저 탭을 연 뒤 첫 탭을 닫아도 남은 탭의 계정은 계속 `온라인`으로 표시됐고 브라우저 오류 0건이다. 마지막 탭 종료 후 타 계정 화면의 오프라인 전환과 비정상 단절 최대 감지시간은 아직 실제 두 계정 E2E로 측정하지 못했다. UI 확인으로 누락된 상태 칩을 추가한 것이 최종 번들 `main.ffa77e1f.js`에 포함됐다.
- 다음 안전 작업: 접근 기록의 정확한 소유자 UID/실제 개인 루트 매핑, 모든 읽기·변경·Drive Agent·공유 다운로드 경로의 감사 이벤트, 외부 변경 전 15일 보존 스냅샷/동시 변경 충돌/권한 재검증, 사용자 전용 조회·알림 UI를 구현/테스트해야 한다. 공개 링크의 익명 다운로더 실명 식별은 불가능하므로 새/기존 링크 로그인 요구 범위에 관한 사용자 응답을 기다린다. 기존 링크를 무단으로 로그인 필수로 바꾸지 않는다.
- 추가 회귀 수정: 관리자 사용자 화면의 30초 전체 갱신이 미저장 역할/할당량 편집을 덮는 문제를 발견했다. 최초 로드에서만 사용자 전체 행을 설정하고 이후 배경 조회는 가입 대기/용량을 갱신하며 경량 presence 응답만 기존 행에 병합한다. 오래된 presence 응답의 역전 적용도 요청 순번으로 차단했다.
- 효율/접근 기록 준비: 기존 개인 활동 로그 조회가 NDJSON 전체를 메모리에 올리던 것을 최대 4MiB 후방 읽기(최대 500개 반환)로 바꾸고 2,500건 로그의 최신순/한글 테스트를 추가했다. 계정 루트는 `/users/<loginId>`의 정확한 경계와 UID로만 판별하고 자기 계정은 외부인 이벤트에서 제외하며 심볼릭 링크가 계정 루트를 벗어나면 거부하는 순수 helper 및 테스트를 추가했다. 아직 실제 탐색/변경 요청에 연결하지 않았고, 활동 기록만 존재한다는 이유로 복원 가능하다고 표시하지 않는다.
- 위 준비 수정은 `c8a3f1b`로 push/NAS fast-forward했다. 백엔드 148건(140 pass/8 skip/0 fail), 프런트 production build, NAS Linux 핵심 3건 모두 통과했다. 이전 index 백업 후 새 번들 `main.c5ebf9a1.js`와 index를 해시 일치로 배포하고 PM2 재시작/save, 내부 HTTP/bundle 200, 공개 관리자 UI에서 자기 계정 온라인·타 계정 오프라인을 다시 확인했다. 이 단계의 외부인 접근 기록 helper는 아직 사용자 노출 기능이 아니다.
- 이어서 접근 기록 1차 구현: `/api/files`의 타인 계정 폴더 탐색을 UID/realpath로 검증한 후 소유자별 `.agent_versions/external-access.ndjson`에 append-only로 기록하고, 같은 폴더의 자동 재조회는 60초 내 중복 억제하되 명시적 탐색은 매번 기록하도록 했다. 첫 접근 알림은 사용자별 방으로 전송하고 동일 방문자·소유자 쌍은 15분 묶는다. 기존 `appendActivity`를 사용하는 파일 변동 중 소유자와 행위자가 다른 항목도 별도 기록한다. 소유자 전용 `/api/access-history`와 좌상단 접근 기록 아이콘/목록을 추가했으며, 정기 폴더 조회와 사용자 탐색의 헤더를 구분했다. 조회는 최신 최대 500건/최대 4MiB만 읽고 계정 메타는 mtime 변경 시에만 다시 읽는다. Windows backend 149건(141 pass/8 skip/0 fail), 프런트 build 성공; 아직 운영 배포/실사용 교차계정 UI 검증 전이다.
- 절대 완료로 표시하지 말 것: 접근 로그는 현재 기존 `appendActivity` 경로에 연결된 변동만 포착하므로 모든 노트·문서·Drive Agent·공유 링크·폴더/파일 전송을 망라하지 않는다. 새 접근 UI의 `restorable:false`는 의도적이다. 변경 전 바이트·폴더 트리의 15일 보존, 충돌 검증, 안전 복원 API/UI, 공개 링크 실명 정책, 드라이브 반영 E2E가 아직 없다. 기존 공개 링크의 권한은 바꾸지 않았다.
- 운영 1차 검증: `c117963` push/NAS ff, Linux 핵심 9건 통과, 새 `main.3d93e975.js` 해시 일치 배포, PM2 online, 내부 backend/bundle 200, 무인증 접근 기록 API 401. 실제 Chrome에서 좌상단 폴더 아이콘이 접근 기록 버튼으로 교체되고 빈 목록이 열리는 것을 확인했다. 마스터로 `/users/admin`과 `/users/admin/NOTE MANAGER`를 차례로 연 후 소유자 전용 기록 파일에 `/`와 `/NOTE MANAGER` 두 행이 남았다. admin 계정으로 알림 수신·자기 기록 UI를 보는 2계정 E2E는 아직 못했다.
- 운영 재점검 발견: 파일 관리자의 3초 자동 갱신 중 하나가 60초 중복 억제 시간이 지나자 관리자 폴더에 세 번째 허위 방문 행을 남겼다. 이를 막기 위해 `?t=` 자동 갱신은 접근 방문으로 기록하지 않고, 실제 폴더 이동과 최초 표시 요청만 `X-NAS-Navigation: 1`로 표시하도록 추가 수정했다. 클라이언트 힌트 기반 방문/배경 구분이므로 악의적 API 클라이언트의 완전한 감사 보증은 아니다. 완료 주장 전에 보안 모델과 통합 테스트를 보강해야 한다.
- 기존 공개 공유 다운로드는 요청 시작 전에 `downloadCount`와 로그를 증가시켜 실패/중단 요청도 성공으로 보이는 결함이 있었다. 정상 HTTP 응답이 `finish`된 뒤에만 카운트와 기존 공유 로그를 쓰도록 바꾸고, 200/206/404/500/조기 close 테스트를 추가했다. 프리뷰는 다운로드 수에 포함하지 않는다. 공유 링크의 익명 방문자 실명 식별/개인별 묶음은 사용자 정책 응답 전에는 구현하지 않았다. Windows 전체 테스트 150건(142 pass/8 skip/0 fail), 프런트 build 성공; 이 추가 수정은 아직 운영 반영 전이다.
- `922fbdf` 운영 반영: NAS Linux 공유 완료 조건 시험 통과, 빌드 `main.28848e32.js` staging/운영 SHA 일치, PM2 online, backend/bundle 200. 브라우저에서 관리자 개인 루트를 다시 열었고 새 탐색 이벤트 한 건이 남았다. 기존 3초 갱신에서 생긴 허위 중복 3건은 삭제/조작하지 않고 정정 사실을 릴레이에 남긴다. 새 배포 후 60초 이상 창을 열어두고 재조회한 결과 소유자 기록 행 수는 6건으로 유지되어 자동 갱신이 더 이상 방문으로 중복 기록되지 않았다.
- 즉시반영/부하 추가 수정: 기존 파일 관리자는 열린 폴더마다 3초 간격 전체 목록 조회를 했다. NAS 파일 변경 활동 후 관련 사용자 소켓 방에 최소 `files:changed` 이벤트를 보내고 프런트는 120ms로 묶어 보이는 폴더를 새로 읽는다. 주기적 조회는 30초 장애 복구용으로 늦췄다. 배경 조회 헤더는 접근 방문 기록에서 제외하고 수동 탐색 헤더는 매번 남긴다. NAS Drive Agent 삭제/충돌 파일 생성 활동 유형도 변경 신호 대상에 포함했다. 코드 시험 150건(142 pass/8 skip/0 fail), 프런트 production build 통과, 운영 반영/실제 타 계정·Windows Explorer 즉시 반영 검증은 아직 전이다.
- 최종 운영 반영/검증: `c6d35d8`를 원격/NAS 활성 브랜치에 ff했고 Linux 핵심 4건 통과. 이전 index 제한 백업 후 `main.ac59b430.js`를 로컬/staging/운영 SHA-256 일치로 반영하고 PM2 재시작·save했다. NAS backend/bundle와 공개 사이트 HTTP 200, Git 양쪽 clean. 공개 Chrome 최신 화면에서 파일 관리자 로딩과 접근 기록 버튼을 확인했고, 현재 번들에서 새 브라우저 오류는 관찰되지 않았다. PM2 재시작 직후 순간치가 아니라 안정화 후 이전 측정은 약 98.8MiB였으나, 변경 전후 장시간 메모리 프로파일은 아직 없어 '모든 동작 메모리 최적'이라고 단정하지 않는다.
- 완료 경계/다음 순서: (1) 사용자 요청대로 마지막 브라우저 탭 종료를 별도 계정 관찰자로 실제 재현하고 네트워크 단절 시간도 재야 한다. (2) 15일 이전 상태 보관과 외부인 변경별 충돌/쿼터/권한 검사 및 한 번 클릭 복원은 **미구현**이다. 현 활동 기록은 `restorable:false`라 복원 버튼을 의도적으로 노출하지 않았다. (3) Note/Office/Driver/공유/이동 등 모든 쓰기 경로를 누락 없이 감사·소유자 알림·즉시반영하도록 전수 연결해야 한다. (4) 공개 공유는 로그인하지 않은 다운로더의 실명을 알 수 없으므로 기존 링크와 새 링크의 로그인 요구 범위를 사용자에게 확인해야 하며, 현재는 기존 공유 권한을 바꾸지 않았다. (5) 서로 다른 사용자 브라우저와 Windows Explorer의 실시간 경로 반영 및 장시간 메모리/CPU 계측을 아직 하지 못했다. 릴레이는 실제 완료와 미완료를 분리한다.

### 2026-09-20 공유 다운로드의 로그인 정책·다운로더 상세 기록

- 사용자 결정: 새로 만드는 공유 링크는 다운로드 때 NAS 로그인을 요구한다. 기존 공개 링크는 권한을 유지하며 비로그인 이용자는 `익명`으로 기록한다. 다운로드 시각과 어떤 파일을 받았는지 표시한다. 로그인된 기존 링크 이용자는 실제 계정으로 기록한다.
- 구현: 새 공유에만 `downloadRequiresLogin=true`를 서버에서 고정한다. 기존 데이터에 이 필드가 없으면 공개 유지한다. 다운로드 4개 경로(단일 파일, 폴더 ZIP, 선택 파일, 선택 ZIP)는 유효한 현재 세션 JWT와 등록된 sessionId가 없으면 401로 차단한다. 미리보기·목록에는 새 로그인 조건을 적용하지 않는다. 완료된 2xx 응답만 다운로더 UID/이름, 시간, 파일 목록, ZIP 파일 수를 공유 로그에 남기고 소유자의 접근 기록에도 `share-downloaded`를 추가한다. 익명 요청은 동일 인물로 추정해 합치지 않는다. 공유 관리 UI는 다운로더별 펼침으로 시각·파일을 보여 주며 새 링크 페이지는 로그인 버튼을 표시한다.
- 효율/한계: ZIP 파일명은 이벤트당 앞의 200개만 공유 로그에 보존하고 전체 개수 및 잘림 여부를 명시한다. 200개 초과 ZIP의 전체 개별 파일명을 조회하는 별도 원장은 아직 없다. 소유자 접근 기록과 공유 로그의 장기 보존·페이지네이션, 다른 계정 실사용 E2E, 다운로드 소유자 알림/Drive 반영, 기존 미완료인 외부 변경 15일 복원은 후속 작업이다. 이 단계에서 완료라고 표시하지 않는다.
...
- 운영·시각 검증: 기능 commit `c0cb52b`를 원격/NAS 활성 브랜치에 fast-forward하고, 로컬 빌드 `main.1eb35207.js`를 staging으로 전송해 SHA-256 일치 확인 후 기존 index를 제한 백업하고 static 먼저/index 마지막 순서로 배포했다. NAS backend HTTP 200, Git clean. 로그인된 실제 Chrome 화면을 새로고침해 오른쪽의 폴더(파일 관리자) → 시계(접근 기록) → 톱니(설정) 아이콘 순서를 스크린샷과 접근성 트리로 확인했다. 새 아이콘 클릭 시 `내 저장공간 접근 기록` 창이 열리고 브라우저 오류 로그는 0건이었다. 기존 파일 관리자와 설정의 기능 코드는 변경하지 않았다.

### 2026-09-20 현재 NAS AI 작업 가능 범위 질문

- 사용자 질문: `ai 작업은 어느정도 가능하니?` 이번 요청은 상태 설명이며 AI 도구 실행, 모델 호출, 파일 변경을 요청하지 않았다.
- 확인: 공개 운영 `/api/ai/status`는 provider `openai`, model `gpt-4.1-mini`, enabled/configured true, toolCount 102, surfaceCount 13을 반환했다. NAS 활성 commit `bcbb6d3`, PM2 `msp-backend` online. 실제 `aiCapabilityCatalog.js`와 `aiAgentRuntime.js`에서 파일·검색·문서 생성/변환·노트·Python/JavaScript 격리 실행·친구·채팅·공유·장치·회의·계정·관리 영역 도구와 작업별 승인 위험 등급을 확인했다.
- 정확한 경계: 읽기 도구와 변경 도구가 섞인 102개 도구 수는 모든 사용자 요청의 E2E 성공률이 아니다. 일반 변경은 현재 승인 모드에 따르고, compute/critical은 `auto_all`에서도 개별 승인이 필요하다. 계정 생성/삭제, 비밀번호/비밀, 영구 삭제, 바이너리 업다운로드, Driver 내부 프로토콜, 실시간 통화 미디어, 임의 코드 패치/명령 실행은 AI에 위임하지 않는다. 과거 대표 실모델 테스트는 전체 기능 검증이 아니며 복잡한 다중 턴/전송/외부 사용자 작업은 개별 실사용 검증이 필요하다.
- 검증/다음: 이번 확인에 유료 AI 모델 호출 0회, 사용자 파일 변경 0건. 기존 workbook의 한글 문자열 손상은 그대로라 안전을 위해 미수정. 다음 안전 작업은 사용자가 원하는 대표 요청을 정해 현재 계정의 실제 승인·실행·결과 확인까지 별도 시험하는 것이다.

### 2026-09-20 AI 요청 시뮬레이션과 안전 경계 보강

- 사용자 요청: 모든 요청의 성공을 보장할 수 있도록 AI 에이전트의 전체 시뮬레이션, 문제 해결, 구현을 진행해 달라는 요청. 무한한 자연어/외부 장애에 대한 100% 보장은 불가능하므로 지원 범위 내 성공, 미지원·모호·실패 시 안전하고 사실에 맞는 응답을 검증 기준으로 삼았다.
- 확인/재현: 기존 102개 도구/13개 제품 영역 catalog와 57개 변경 도구의 질문 행렬을 확인했다. 미완성 작업 후 짧은 금지문, 새 명령, 조회 요청이 이전 변경 권한을 이어받을 수 있었고, 문서 형식만 답한 후속 요청이 별도 Office 생성으로 분류될 수 있었다. 승인 카드가 없는데 승인하라는 허위 응답 차단은 있었지만, 실제 변경 도구가 성공하지 않았는데 모델이 완료했다고 말하는 경우는 차단하지 않았다.
- 수정: 미완성 작업의 권한 승계를 취소·금지·조회·과거 질문·무관 새 명령에서 차단하고, 유효 시간 내 간단한 동의/대상/문서 형식·위치 보충만 이어지도록 좁혔다. 원래 요청에 대상자가 이미 있으면 다른 대상에게 보내 달라는 문장을 묵시 승계하지 않는다. 직접 변경 작업을 허가한 요청에서 모델이 완료를 주장하면 허가된 변경 도구 전부의 실제 completed 이벤트를 요구하고, 없으면 일부 실행 가능성을 알리며 완료 주장을 거둔다.
- 검증: 57개 변경 도구 각각에 대해 미완성 대화의 취소·금지·무관 명령 시뮬레이션을 추가했다. 전체 Windows backend 155건(147 pass, 8 환경 skip, 0 fail), 변경 파일 Node 구문 검사와 git diff --check 통과. 유료 OpenAI 모델 호출 0회, 실제 사용자 파일·채팅·외부 계정에 대한 시험 변경 0건.
- 남은 경계: 이는 결정적 코드/모의 테스트이며 모든 자연어, 사용자 승인, 사용자별 권한, 실서비스 도구 102종의 실제 E2E 성공 보증이 아니다. 복합 요청의 부분 성공/재시도, 모델의 완곡한 허위 완료 표현, 브라우저·PC 별 UI와 서비스 장애 복구는 실제 격리 시험이 더 필요하다. 기존 canonical xlsx의 README 및 제어 시트가 replacement 문자로 이미 손상돼 있어 안전하게 덮어쓰지 않았고, 복원 후 Request_Archive/Patch_Log/AI 관련 시트에 반영해야 한다.
- 운영 반영: `b65c752`를 GitHub와 NAS 활성 브랜치에 fast-forward했다. NAS Linux AI 핵심 25건 통과, 변경 JavaScript 구문 검사 통과, PM2 reload/save 후 online, NAS Git clean, 내부 `/api/ai/status`와 공개 사이트 모두 HTTP 200. reload 직후 1회 HTTP 연결 실패는 프로세스 시작 전 순간 상태였고 재시도 시 200/online을 확인했다. PM2 로그의 기존 Office proxy socket hang up 및 이동된 PDF 경로 ENOENT는 이번 AI 변경과 무관한 별도 현상으로 원인을 확인하지 않았으며 이 요청에서 수정하지 않았다.

### 2026-09-20 AI 사진 열거·ZIP 다운로드·PC/NAS 첨부·비중첩 사이드바

- 사용자 요청: 예컨대 자바 시험 관련 문서들을 찾아 ZIP으로 다운로드하고 자기 경로의 모든 사진 이름을 열거하는 에이전트 동작, PC/NAS의 사진·파일 붙여넣기·첨부 버튼·드래그, AI 사이드바가 바탕화면을 덮거나 클릭 시 닫히지 않고 작업 공간을 밀어내는 배치를 원함. 모든 무한한 자연어 요청의 성공 보장은 불가능하며 지원 범위·미확인/일부 결과를 명시해야 한다.
- 구현: 계정 경계·숨김/민감 파일 제외·항목/용량 제한·원본 재검증을 거치는 ZIP 승인/다운로드 흐름과 사진 경로 열거 도구(페이지, 완료 여부, 스캔/접근 실패 표시)를 추가했다. AI 패널을 메인 영역과 flex 병렬로 배치하고 NAS 선택·드래그, PC 선택·붙여넣기·드래그를 추가했다. 최대 3개/합계 320KB의 지원 이미지·PDF·Office·UTF-8 텍스트/코드 본문만 모델 입력에 포함한다. NAS 폴더, 미지원 형식, 제한 초과 파일은 경로 정보만 전달하고 본문을 읽었다고 주장하지 않도록 했다. ZIP은 승인 후 별도 다운로드하며 만료/변경된 원본이면 중단한다.
- 검증 진행: Windows backend 전체 158건(150 pass, 8 환경 skip, 0 fail), 프런트 production build 및 PDF.js 호환성 검사 통과. 유료 모델 호출과 실계정 ZIP 다운로드 E2E는 아직 안 했다. 관련 문서를 의미상 빠짐없이 찾는 것, 스캔 상한 밖 파일, 큰 파일/HEIC 등의 모델 직접 분석은 아직 보장되지 않는다. 기존 workbook 한국어 문자열 손상 때문에 `docs/NAS_PROJECT_LOG.xlsx`는 복구 전까지 보존한다. 배포와 실제 화면 확인 결과는 후속 항목으로 기록한다.
- 최초 운영 반영: `a0e1507`을 GitHub와 NAS 활성 브랜치에 fast-forward했다. NAS 신규 핵심 테스트 7건 통과, 로컬 build `main.c9868676.js`와 index의 staging/운영 SHA-256 일치, PM2 reload/save 후 online, 내부 AI status·번들 및 공개 HTTPS 200, AI toolCount 104, NAS Git clean을 확인했다. Windows 화면 제어는 현재 브라우저 URL을 정책상 확실히 판별하지 못해 도구가 중단했으므로 실제 화면 조작 검증을 완료했다고 주장하지 않는다.

### 2026-09-20 AI 첨부·사진 조회·ZIP 배포 후 보안/오류 감사

- 사용자 요청: 앞선 AI 기능 전체의 오류·보안 문제를 검토하고 마무리해 달라는 요청. 기존 변경 `a0e1507`, 계정 경계, 승인 시점·다운로드 시점, 저장 상태, 사진 조회, 파일 드래그/첨부를 다시 확인했다.
- 확인/수정: ZIP 승인 당시의 파일 목록을 실행 시점에 재계산한 목록과 완전히 비교하여 새 파일·변경 파일이 승인 없이 포함되지 않도록 했다. ZIP 내부 이름의 Windows 경로 문자·`..`·역슬래시를 거절하고 기록된 manifest도 다시 검증한다. 다운로드와 NAS 첨부는 열린 파일 descriptor의 크기·수정 시각·Linux `/proc/self/fd` 실제 경로를 검증해 경로 교체 경쟁을 줄였다. AI 파일 목록/검색은 숨김·민감 이름과 symlink를 제외한다. ZIP 성공 시 타 계정 소유자별 접근 기록/알림, 타 계정 파일을 AI에 첨부해 읽을 때 접근 기록/알림을 추가했다. UI는 첨부 초과 개수를 조용히 버리지 않고 오류를 알리며, 연속 첨부 상태 경합과 폴더·제한 초과 항목의 본문 미전송 안내를 바로잡았다.
- 검증 진행: Windows backend 전체 158건(150 pass, 8 환경 skip, 0 fail), 변경 JS 구문 검사, frontend production build 및 PDF.js gate 통과. Linux 고유 ZIP 파일명 회귀, 실제 인증 세션의 첨부→모델→승인→다운로드, 타 계정 알림, 중간 파일 변경/전원 중단 시 다운로드, 브라우저/Windows 파일 드래그 E2E는 아직 확인 전이다. 모델 유료 호출과 실제 사용자 데이터 변경은 하지 않았다.
- 잔여 위험: 기존 AI의 일반 `list_files`/`search_files`/`read_text_file`로 마스터/관리자가 타 계정 경로를 조회하는 동작은 소유자 알림 원장과 완전히 통합되지 않았다. 새 ZIP과 직접 첨부는 보강했지만 전체 AI 읽기 경로의 접근 기록/알림은 별도 통합 검증이 필요하다. 동시 수정 중 ZIP 바이트 일관성은 파일 descriptor 고정과 사전 검사로 완화했으나 전체 파일 스냅샷을 만들지는 않는다. 의미 검색의 전수성·모든 자연어 요청 성공은 보장 불가. 손상된 workbook은 원본 보존하고 업데이트하지 않았다.
- Linux 회귀 테스트 교정: 첫 감사 커밋 `d58b1a0`의 ZIP 역슬래시 이름 테스트가 Linux에서 종료코드 14로 실패했다. 테스트가 자식 `node -e` 스크립트를 템플릿 문자열로 생성하며 역슬래시가 두 번 파싱되어 검증하려던 실제 파일명이 달라진 것이 원인이다. 파일명에 `String.fromCharCode(92)`로 역슬래시를 생성하도록 테스트를 교정했다. 교정 테스트 재실행·배포 확인 전까지 Linux 통과로 기록하지 않는다.
- 추가 교정: `3c6c870`에서도 종료코드 14가 반복되어 테스트 대상 파일명이 `..`으로 시작해 기존 숨김 파일 제외 규칙에 의해 정상적으로 건너뛰어진 점을 확인했다. 첫 진단의 이스케이프 설명만으로는 원인을 다 설명하지 못했으며, 보이는 이름 안에 역슬래시와 `..`이 들어가도록 바꿔 실제 ZIP 이름 검증 분기를 시험한다.
- 최종 운영 검증: `25e9d52`까지 GitHub/NAS 활성 브랜치에 fast-forward했고 Linux 핵심 ZIP·첨부·소유자 접근 테스트 6/6, NAS backend 전체 158건 중 152 pass/6 환경 skip/0 fail을 확인했다. 로컬 build `main.6989ef11.js`와 index의 staging/운영 SHA-256이 각각 일치하고 PM2 reload/save 후 online이다. 내부 AI status·새 JS bundle·공개 HTTPS는 200, 비로그인 ZIP 다운로드는 401, toolCount 104, NAS checkout clean을 확인했다. 운영 로그인 세션에서 PC/NAS 첨부→실제 유료 모델→승인→ZIP 저장, 다른 계정 알림, 브라우저 드래그·레이아웃은 직접 재현하지 못했다. 다음 안전 작업은 별도 테스트 계정/샘플 파일을 사용한 이 E2E와 기존 AI 조회 도구의 외부 소유자 접근 기록 통합이다.
