...
const MUTATION_INTENT_RULES = {
  ...
  create_notebook: /(?:노트북|프로젝트(?:\s*작업공간)?).*(?:생성|만들)|(?:생성|만들).*(?:노트북|프로젝트(?:\s*작업공간)?)/i,
...
const TOOL_DEFINITIONS = [
  ...
  { type: 'function', name: 'create_notebook', description: 'Note Studio에 Notion식 노트북 또는 코드·파일 실행 중심 프로젝트 작업공간을 만든다.', strict: true, parameters: schema({ title: stringProp('노트북 또는 프로젝트 이름'), kind: { type: 'string', enum: ['notes', 'project'] } }, ['title', 'kind']) },
...
const actionSpec = (name, args) => {
  const map = {
    ...
    create_notebook: { title: args.kind === 'project' ? '프로젝트 작업공간 생성' : '노트북 생성', risk: 'safe', actionType: name, notebookPayload: { title: args.title, kind: args.kind } },
