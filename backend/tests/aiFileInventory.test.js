...
test('사진 목록 페이지와 ZIP 계획은 계정 밖·숨김 경로를 제외하고 변경된 원본을 거절한다', () => {
  ...
  const script = `
    ...
    if (bundle.files.length !== 3 || bundle.files.some((item) => item.name.includes('.secret'))) process.exit(3);
    ...
    const completed = await runtime.executeAction(user, planned.actionId, { platformCall: async () => ({}) });
    if (completed.status !== 'completed' || completed.result.fileCount !== 3 || !completed.result.downloadUrl.includes(planned.actionId)) process.exit(8);
