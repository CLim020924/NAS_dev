...
# SidebarTree.js 내용 (느낌표 완벽 보호)
sidebar_tree_content = """import React, { useState, useEffect, useRef } from 'react';
...
# NASWindow.js 내용 (DIRECTORY TREE 글자 포함)
nas_window_content = """import React, { useEffect } from 'react';
...
const NASWindow = ({ win, isMobile, theme, focusedContext, focusWindow, setOpenWindows, toggleSidebar, handleUp, fetchFiles, toggleMinimize, toggleMaximize, closeWindow, handleDragOver, handleDrop, handleContextMenu, setSelectedItems, inlineEdit, selectedItems, dragOverTarget, handleDragStart, handleDragLeave, handleItemClick, openFileWindow, handleInlineSubmit, setInlineEdit, InlineInput, toggleEditMode, saveFile, handleContentChange }) => {
  ...
  return (
    <Rnd key={win.id} style={{ zIndex: win.zIndex, position: isMobile ? 'fixed' : 'absolute' }} disableDragging={isMobile || win.isMaximized} size={isMobile ? { width: '100%', height: '100%' } : { width: winStyles.width, height: winStyles.height }} position={isMobile ? { x: 0, y: 0 } : { x: winStyles.x, y: winStyles.y }} onMouseDown={() => focusWindow(win.id)} dragHandleClassName='window-header-drag-handle' bounds='parent'>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ height: '100%', width: '100%' }}>
        <Paper elevation={isActive ? 24 : 8} sx={{ height: '100%', display: 'flex', flexDirection: 'column', borderRadius: 2, overflow: 'hidden', background: theme.palette.background.paper, border: isActive ? '3px solid ' + theme.palette.primary.main : '1px solid ' + theme.palette.divider }}>
...
with open('src/components/NAS/Window/SidebarTree.js', 'w', encoding='utf-8') as f: f.write(sidebar_tree_content)
with open('src/components/NAS/Window/NASWindow.js', 'w', encoding='utf-8') as f: f.write(nas_window_content)
