...
const TopBar = ({ onOpenMessages }) => {
  ...
  const handlePasswordChange = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        handleLogout(); // 보안을 위해 비번 변경 후 로그아웃 처리
      }
    } catch (err) {
  ...
  const appbarBg = isDesktop ? 'rgba(0,0,0,0.2)' : (themeName === 'dark' ? '#0f172a' : '#fff');
  const textColor = (isDesktop || themeName === 'dark') ? '#fff' : '#1e293b';

  return (
    <>
      <AppBar position="fixed" elevation={isDesktop ? 0 : 2} sx={{ zIndex: (theme) => theme.zIndex.drawer + 1, backgroundColor: appbarBg, backdropFilter: isDesktop ? 'blur(10px)' : 'none', color: textColor }}>
        <Toolbar size="small" sx={{ minHeight: '48px !important' }}>
          ...
          <Chip label={getPageTitle(location.pathname)} size="small" sx={{ ml: 2, backgroundColor: isDesktop ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)', color: textColor }} />

          <Box sx={{ flexGrow: 1 }} />

          <Box sx={{ display: 'flex', gap: 0.5 }}>
            <IconButton onClick={onOpenMessages} size="small" sx={{ color: textColor }}>
              <Badge color="error" variant="dot"><MessageIcon fontSize="small" /></Badge>
            </IconButton>

            <IconButton onClick={() => navigate('/nas')} size="small" sx={{ color: textColor }}> <FolderIcon fontSize="small" /> </IconButton>
            
            <IconButton onClick={() => navigate('/settings')} size="small" sx={{ color: textColor }}> <SettingsIcon fontSize="small" /> </IconButton>

            <IconButton onClick={(e) => setAnchorEl(e.currentTarget)} size="small">
              <Avatar sx={{ width: 28, height: 28, fontSize: '0.8rem', bgcolor: 'primary.main', fontWeight: 'bold' }}>{user.username?.[0]?.toUpperCase()}</Avatar>
            </IconButton>
          </Box>
          ...
          <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)} PaperProps={{ elevation: 3, sx: { mt: 1, minWidth: 150, borderRadius: 2 } }}>
            ...
            <Box sx={{ my: 0.5, borderBottom: '1px solid #e2e8f0' }} />
