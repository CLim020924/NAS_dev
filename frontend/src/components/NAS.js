...
const NAS = ({ showWorkspace = true }) => {
  ...
  useEffect(() => { openWindows.forEach(w => { if (w.winType === 'folder' && !w.isLoaded) fetchFiles(w.id, w.currentPath, { navigation: true }); }); }, [openWindows, fetchFiles]);
  ...
  const handleUp = (win) => { if (win.currentPath === win.basePath || win.currentPath === '/') return; const segments = win.currentPath.split('/').filter(Boolean); segments.pop(); fetchFiles(win.id, segments.length > 0 ? ensureSlash(segments.join('/')) : '/', { navigation: true }); };
          ...
          return (
            ...
            >
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} style={{ height: '100%', width: '100%' }}>
                <Paper elevation={0} sx={{ height: '100%', display: 'flex', flexDirection: 'column', borderRadius: (isMobile || win.isMaximized || win.isImmersive) ? 0 : 1.5, overflow: 'hidden', bgcolor: 'background.paper', border: isActive ? `1px solid ${alpha(theme.palette.primary.main, 0.62)}` : `1px solid ${theme.palette.divider}`, boxShadow: isActive ? `0 12px 34px ${alpha(theme.palette.common.black, theme.palette.mode === 'dark' ? 0.38 : 0.12)}` : `0 8px 24px ${alpha(theme.palette.common.black, theme.palette.mode === 'dark' ? 0.28 : 0.08)}`, transition: 'border-color 120ms ease, box-shadow 120ms ease' }}>
                  ...
                  <Box className="window-header-drag-handle" onMouseEnter={() => setHoveredHeader(win.id)} onMouseLeave={() => setHoveredHeader(null)} sx={{ display: 'flex', position: win.isImmersive ? 'absolute' : 'relative', top: win.isImmersive ? (hoveredHeader === win.id ? 0 : '-70px') : 0, left: 0, right: 0, width: '100%', transition: 'top 0.35s cubic-bezier(0.4, 0, 0.2, 1)', zIndex: 100000, px: isMobile ? 1 : 1.25, py: isMobile ? 0.75 : 1, background: isActive ? alpha(theme.palette.primary.main, 0.08) : theme.palette.background.paper, borderBottom: `1px solid ${theme.palette.divider}`, justifyContent: 'space-between', alignItems: 'center', flexShrink: 0, cursor: win.isMaximized || isMobile ? 'default' : 'move', minHeight: isMobile ? 52 : 48 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, overflow: 'hidden', minWidth: 0, flex: 1 }}>
                      {win.winType === 'folder' ? (
                        <>
                          <IconButton size="small" aria-label="사이드바 전환" title="사이드바 전환" onClick={() => toggleSidebar(win.id)} color={isActive ? "primary" : "inherit"} onMouseDown={(e) => e.stopPropagation()}><MenuIcon /></IconButton>
                          <IconButton size="small" aria-label="상위 폴더" title="상위 폴더" onClick={() => handleUp(win)} disabled={win.currentPath === win.basePath} color="inherit" onMouseDown={(e) => e.stopPropagation()}><ArrowBackIcon /></IconButton>
                          <Box sx={{ display: 'flex', alignItems: 'center', flexWrap: 'nowrap', overflowX: 'auto', ml: isMobile ? 0.5 : 1, minWidth: 0 }} onMouseDown={(e) => e.stopPropagation()}>
                            <Typography title={win.name} onClick={() => fetchFiles(win.id, win.basePath, { navigation: true })} sx={{ cursor: 'pointer', '&:hover': { textDecoration: 'underline' }, fontWeight: 800, whiteSpace: 'nowrap' }}>{win.name}</Typography>
                            {getRelativeSegments(win.currentPath, win.basePath).map((seg, idx, arr) => (<React.Fragment key={idx}><Typography sx={{ mx: 0.5, color: 'text.secondary' }}>/</Typography><Typography onClick={() => fetchFiles(win.id, (win.basePath === '/' ? '' : win.basePath.replace(/\/$/, '')) + '/' + arr.slice(0, idx + 1).join('/'), { navigation: true })} sx={{ cursor: 'pointer', '&:hover': { textDecoration: 'underline' }, fontWeight: 800, whiteSpace: 'nowrap' }}>{seg}</Typography></React.Fragment>))}
                      ...
                      ) : (<><InsertDriveFileIcon color={isActive ? "primary" : "inherit"} /><Typography className="nas-dynamic-label" title={win.name} sx={{ fontWeight: 800 }}>{win.name}</Typography></>)}
                    
</Box>

                    <Box sx={{ display: 'flex', flexShrink: 0 }} onMouseDown={(e) => e.stopPropagation()}>
                      {!isMobile && <IconButton size="small" aria-label="최소화" title="최소화" onClick={(e) => { e.stopPropagation(); if (win.isImmersive) { setOpenWindows(prev => prev.map(w => w.id === win.id ? { ...w, isImmersive: false, isMinimized: true } : w)); } else { toggleMinimize(win.id); } }}><RemoveIcon fontSize="small"/></IconButton>}
                      {!isMobile && <IconButton size="small" aria-label={win.isMaximized ? '창 복원' : '최대화'} title="최대화 / 길게 누르면 전체화면" onClick={(e) => { e.stopPropagation(); if (win.isImmersive) { setOpenWindows(prev => prev.map(w => w.id === win.id ? { ...w, isImmersive: false } : w)); } else { toggleMaximize(win.id); } }} onMouseDown={() => { window.immersiveTimer = setTimeout(() => { setOpenWindows(prev => prev.map(w => w.id === win.id ? { ...w, isImmersive: true } : w)); }, 500); }} onMouseUp={() => clearTimeout(window.immersiveTimer)} onMouseLeave={() => clearTimeout(window.immersiveTimer)} onTouchStart={() => { window.immersiveTimer = setTimeout(() => { setOpenWindows(prev => prev.map(w => w.id === win.id ? { ...w, isImmersive: true } : w)); }, 500); }} onTouchEnd={() => clearTimeout(window.immersiveTimer)}>{win.isMaximized ? <FilterNoneIcon fontSize="small"/> : <CropSquareIcon fontSize="small"/>}</IconButton>}
                      <IconButton size="small" aria-label="닫기" title="닫기" onClick={(e) => { e.stopPropagation(); handleCloseWindowClick(win); }} color="error"><CloseIcon fontSize="small"/></IconButton>
