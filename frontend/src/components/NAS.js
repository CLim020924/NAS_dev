                      ...
                      {win.winType === 'folder' ? (
                        <>
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', flexWrap: 'nowrap', overflowX: 'auto', ml: isMobile ? 0.5 : 1, minWidth: 0 }} onMouseDown={(e) => e.stopPropagation()}>
                            <Typography title={win.name} onClick={() => fetchFiles(win.id, win.basePath)} sx={{ cursor: 'pointer', '&:hover': { textDecoration: 'underline' }, fontWeight: 800, whiteSpace: 'nowrap' }}>{win.name}</Typography>
