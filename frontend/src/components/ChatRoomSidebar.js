...
const ChatRoomSidebar = ({
  ...
  const handleConversationClick = (conversation) => {
    ...
    onActiveChatChange(
      activeChat?.conversationId === next.conversationId ? null : next
    );
  };
  ...
  const openWorkspaceWindow = (conversationId = null) => {
    ...
    const workspaceWidth = Math.max(480, Math.min(980, viewportWidth - 64));
    const workspaceHeight = Math.max(420, Math.min(680, viewportHeight - 120));
    const workspaceX = Math.max(24, Math.round((viewportWidth - workspaceWidth) / 2));
    const workspaceY = Math.max(24, Math.min(72, Math.round((viewportHeight - workspaceHeight) / 2)));
    ...
    setOpenWindows((prev) => [
      ...prev,
      {
        ...
        workspaceSelectedConversationId: conversationId || activeChat?.conversationId || null,
        ...
        >
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            ...
            <Button
              ...
              onClick={() => openWorkspaceWindow(activeChat?.conversationId || null)}
            >
              window
            </Button>
