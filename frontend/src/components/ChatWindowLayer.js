...
const ChatWindowLayer = () => {
  ...
  useEffect(() => {
    ...
    openWindows.forEach((w) => {
      if (w.winType === 'chat' && w.chatMode === 'window') {
        const key = w.chatUserUid || w.id;
        if (seen.has(key)) {
          changed = true;
          return;
  ...
  useEffect(() => {
    if (windowChats.length === 0) return;
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', minWidth: 0, gap: 1 }}>
                    ...
                    <Box sx={{ minWidth: 0 }}>
                      ...
                      <Typography variant="caption" color="text.secondary" noWrap>
                        {win.chatDisplayName && win.chatDisplayName !== win.chatUsername ? win.chatDisplayName : (win.chatRole || '')}
                      </Typography>
                    </Box>
