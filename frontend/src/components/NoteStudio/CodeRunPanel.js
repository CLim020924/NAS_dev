...
const stateLabel = { queued: '대기 중', starting: '시작 중', running: '실행 중', finished: '완료', failed: '오류', blocked: '차단됨', cancelled: '취소됨', stopped: '중지됨' };

const CodeRunPanel = ({ session, events, onInput, onStop, onHide }) => {
  const [input, setInput] = useState('');
  const [expanded, setExpanded] = useState(false);
  const outputRef = useRef(null);
  const running = ['queued', 'starting', 'running'].includes(session?.state);
  ...
  return <Box data-testid="code-run-panel" sx={{ flex: expanded ? '0 1 min(42%, 420px)' : '0 1 clamp(140px, 24%, 220px)', minHeight: 116, maxHeight: '42%', display: 'flex', flexDirection: 'column', bgcolor: '#0d1117', color: '#d8dee9', borderTop: '1px solid #2a313c', transition: 'flex-basis 140ms ease' }}>
    <Stack direction="row" alignItems="center" spacing={1} sx={{ minHeight: 40, px: 1.25, bgcolor: '#11161e', borderBottom: '1px solid #2a313c' }}>
      ...
      <Typography variant="caption" noWrap sx={{ maxWidth: '45%', color: '#8390a1' }}>{session?.displayName || '프로그램'} · {session?.noteTitle || '코드 페이지'}{session?.state === 'queued' ? ` · 대기 ${session.position || 1}번째` : ' · 표준 출력/입력'}</Typography>
      <Box sx={{ flex: 1 }} />
      ...
      {running && <Tooltip title={session?.state === 'queued' ? '실행 대기 취소' : '실행 중지'}><IconButton size="small" onClick={onStop} sx={{ color: '#ff9a9a' }}><StopIcon fontSize="small" /></IconButton></Tooltip>}
    ...
    <Stack direction="row" alignItems="center" spacing={1} sx={{ px: 1.25, py: 0.75, bgcolor: '#11161e', borderTop: '1px solid #2a313c' }}>
      ...
      <TextField
        ...
        value={input} disabled={!running || session?.state !== 'running' || session?.interactive === false} onChange={(event) => setInput(event.target.value)}
      ...
      />
      <Button size="small" disabled={!running || session?.state !== 'running' || session?.interactive === false} onClick={submit} sx={{ color: '#b7c3d2' }}>입력</Button>
    </Stack>
  </Box>;
};
