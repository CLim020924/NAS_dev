...
const NoteStudio = () => {
  ...
  const createNote = async (option) => {
    ...
    try {
      const { data } = await axios.post('/api/note-studio/notes', { title: `새 ${option.label}`, type: option.type, language: option.type === 'code' ? 'plaintext' : '' }, { withCredentials: true });
      setTrashMode(false);
    ...
    <Box sx={{ height: '100%', minHeight: 0, display: 'grid', gridTemplateColumns: { xs: '1fr', md: '270px minmax(0, 1fr)' }, bgcolor: 'background.default' }}>
      <Paper square elevation={0} sx={{ display: { xs: selected ? 'none' : 'flex', md: 'flex' }, minHeight: 0, flexDirection: 'column', borderRight: { md: '1px solid' }, borderColor: 'divider' }}>
        <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ p: 1.5 }}>
          ...
          <Tooltip title="새 노트"><IconButton color="primary" onClick={(event) => setCreateAnchor(event.currentTarget)}><AddIcon /></IconButton></Tooltip>
        </Stack>
        ...
        <Box sx={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
          {loading ? <Box sx={{ py: 5, display: 'grid', placeItems: 'center' }}><CircularProgress size={24} /></Box> : notes.length === 0 ? <Box sx={{ p: 3, textAlign: 'center' }}><Typography color="text.secondary" variant="body2">{query ? '검색 결과가 없습니다.' : trashMode ? '휴지통이 비어 있습니다.' : '새 노트를 만들어 시작하세요.'}</Typography></Box> : <List dense disablePadding>{notes.map((note) => {
            const type = TYPE_OPTIONS.find((option) => option.type === note.type) || TYPE_OPTIONS[0];
            const Icon = type.Icon;
            return <ListItemButton key={note.id} selected={selected?.id === note.id} onClick={() => openNote(note)} sx={{ py: 1, borderBottom: '1px solid', borderColor: alpha(theme.palette.divider, 0.7) }}><ListItemIcon sx={{ minWidth: 34 }}><Icon fontSize="small" /></ListItemIcon><ListItemText primary={note.title} secondary={`${type.label} · ${formatTime(note.updatedAt)}`} primaryTypographyProps={{ noWrap: true, fontWeight: 750 }} secondaryTypographyProps={{ noWrap: true, fontSize: 11 }} /></ListItemButton>;
          })}</List>}
        </Box>
      ...
      <Menu anchorEl={createAnchor} open={!!createAnchor} onClose={() => setCreateAnchor(null)}>{TYPE_OPTIONS.map((option) => <MenuItem key={option.type} onClick={() => createNote(option)} sx={{ py: 1.25, minWidth: 250 }}><ListItemIcon><option.Icon fontSize="small" /></ListItemIcon><ListItemText primary={option.label} secondary={option.detail} /></MenuItem>)}</Menu>
