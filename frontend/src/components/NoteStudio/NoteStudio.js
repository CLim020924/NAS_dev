...
const NoteStudio = () => {
  ...
  useEffect(() => {
    ...
    sessionViewStateQueueRef.current.schedule({ kind: 'note-studio-session' }, {
      ...
      collapsedNotebookIds: [...collapsedNotebookIds], terminalOpen,
      terminalNotebookId: terminalNotebookId || '', terminalExplorerOpen, sidebarOpen,
    });
  }, [activeNotebookId, collapsedNotebookIds, selected?.id, sessionReady, sidebarOpen, terminalExplorerOpen, terminalNotebookId, terminalOpen]);
  ...
  const toggleNotebookCollapsed = (notebookId) => {
  ...
  return (
    <Box className="note-studio-shell" sx={{ height: '100%', minHeight: 0, display: 'grid', gridTemplateColumns: sidebarOpen ? { xs: '1fr', md: '270px minmax(0, 1fr)' } : '42px minmax(0, 1fr)', bgcolor: 'background.default' }}>
      ...
      <Paper square elevation={0} sx={{ display: { xs: sidebarOpen && !(selected || terminalOpen) ? 'flex' : 'none', md: sidebarOpen ? 'flex' : 'none' }, minHeight: 0, flexDirection: 'column', borderRight: { md: '1px solid' }, borderColor: 'divider' }}>
        <Stack direction="row" alignItems="center" justifyContent="flex-end" className="note-studio-compact-controls" sx={{ minHeight: 38, px: 0.75, borderBottom: '1px solid', borderColor: 'divider' }}>
          <Tooltip title="목록 숨기기 · Ctrl+B"><IconButton size="small" aria-label="노트북 목록 숨기기" onClick={() => setSidebarOpen(false)}><ViewSidebarOutlinedIcon fontSize="small" /></IconButton></Tooltip><Tooltip title="모든 노트북 접기"><span><IconButton size="small" disabled={!notebooks.length} onClick={() => setCollapsedNotebookIds(new Set(notebooks.map((item) => item.id)))}><UnfoldLessIcon fontSize="small" /></IconButton></span></Tooltip><Tooltip title="선택한 노트북에서 터미널 열기"><span><IconButton size="small" disabled={!activeNotebookId} onClick={() => openNotebookTerminal()}><TerminalIcon fontSize="small" /></IconButton></span></Tooltip><Tooltip title="새 노트북"><IconButton size="small" onClick={() => setNotebookDialogOpen(true)}><CreateNewFolderOutlinedIcon fontSize="small" /></IconButton></Tooltip><Tooltip title="새 페이지"><IconButton size="small" color="primary" onClick={(event) => { setPendingParentId(null); if (!activeNotebookId) setNotebookDialogOpen(true); else setCreateAnchor(event.currentTarget); }}><AddIcon fontSize="small" /></IconButton></Tooltip>
        </Stack>
        <Box sx={{ px: 1, py: 0.75 }}><TextField value={query} onChange={(event) => setQuery(event.target.value)} size="small" fullWidth placeholder="검색" InputProps={{ startAdornment: <SearchIcon fontSize="small" sx={{ mr: 0.75, color: 'text.secondary' }} /> }} /></Box>
        ...
        <Box sx={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
          {loading ? <Box sx={{ py: 5, display: 'grid', placeItems: 'center' }}><CircularProgress size={24} /></Box> : notes.length === 0 && notebooks.length === 0 ? <Box sx={{ p: 3, textAlign: 'center' }}><Typography color="text.secondary" variant="body2">{query ? '검색 결과가 없습니다.' : trashMode ? '휴지통이 비어 있습니다.' : '새 노트북을 만들어 시작하세요.'}</Typography></Box> : <List dense disablePadding>{notesByNotebook.map(({ notebook, notes: notebookNotes }) => <Box key={notebook.id}><ListItemButton selected={activeNotebookId === notebook.id} onClick={() => setActiveNotebookId(notebook.id)} onDoubleClick={() => openNotebookTerminal(notebook.id)} sx={{ py: 0.85, bgcolor: alpha(theme.palette.text.primary, 0.025) }}><IconButton size="small" aria-label={collapsedNotebookIds.has(notebook.id) ? `${notebook.title} 펼치기` : `${notebook.title} 접기`} onClick={(event) => { event.stopPropagation(); toggleNotebookCollapsed(notebook.id); }} sx={{ mr: 0.25 }}><KeyboardArrowRightIcon fontSize="small" sx={{ transform: collapsedNotebookIds.has(notebook.id) ? 'rotate(0deg)' : 'rotate(90deg)', transition: 'transform 120ms ease' }} /></IconButton><ListItemIcon sx={{ minWidth: 31 }}>{notebook.kind === 'project' ? <TerminalIcon fontSize="small" /> : <MenuBookOutlinedIcon fontSize="small" />}</ListItemIcon><ListItemText primary={notebook.title} secondary={notebook.available === false ? `${notebook.directoryName} · 경로 확인 필요` : `${notebook.kind === 'project' ? '프로젝트' : '노트'} · ${notebookNotes.length}개 페이지`} primaryTypographyProps={{ fontWeight: 900, noWrap: true }} secondaryTypographyProps={{ fontSize: 10, noWrap: true, color: notebook.available === false ? 'error' : 'text.secondary' }} /></ListItemButton>{!collapsedNotebookIds.has(notebook.id) && notebookNotes.map((note) => {
      ...
      <Box sx={{ minWidth: 0, minHeight: 0, display: { xs: selected || terminalOpen ? 'flex' : 'none', md: 'flex' }, flexDirection: 'column' }}>
        ...
        {!selected ? <Box sx={{ flex: 1, minHeight: 0, display: 'grid', placeItems: 'center', p: 3 }}><Box sx={{ textAlign: 'center', maxWidth: 480 }}>{activeNotebook?.kind === 'project' ? <TerminalIcon sx={{ fontSize: 60, color: alpha(theme.palette.text.primary, 0.22) }} /> : <NotesIcon sx={{ fontSize: 64, color: alpha(theme.palette.text.primary, 0.22) }} />}<Typography variant="h6" sx={{ mt: 1, fontWeight: 900 }}>{activeNotebook?.kind === 'project' ? '프로젝트 작업공간' : '노트북 안에 페이지를 구성하세요'}</Typography><Typography color="text.secondary" sx={{ mt: 0.75 }}>{activeNotebook?.kind === 'project' ? '코드·설정·데이터 파일을 한 폴더에 두고 하단 실행 콘솔과 노트북 터미널에서 함께 작업합니다.' : '블록, Markdown, TXT를 Notion식 페이지 트리로 관리하고 필요할 때 코드와 실제 파일을 연결합니다.'}</Typography><Stack direction="row" spacing={1} justifyContent="center" sx={{ mt: 2 }}>{activeNotebook?.kind === 'project' && <Tooltip title="프로젝트 터미널 열기"><Button variant="contained" startIcon={<TerminalIcon />} onClick={() => openNotebookTerminal()}>터미널</Button></Tooltip>}<Button variant={activeNotebook?.kind === 'project' ? 'outlined' : 'contained'} startIcon={notebooks.length ? <AddIcon /> : <CreateNewFolderOutlinedIcon />} onClick={(event) => { setPendingParentId(null); if (!activeNotebookId) setNotebookDialogOpen(true); else setCreateAnchor(event.currentTarget); }}>{notebooks.length ? '새 페이지' : '새 노트북'}</Button>{activeNotebookId && activeNotebook?.kind !== 'project' && <Button variant="outlined" startIcon={<TerminalIcon />} onClick={() => openNotebookTerminal()}>터미널</Button>}</Stack></Box></Box> : <>
          <Stack direction="row" alignItems="center" spacing={0.5} className="note-studio-page-bar" sx={{ px: 0.75, minHeight: 40, maxHeight: 40, borderBottom: '1px solid', borderColor: 'divider', bgcolor: 'background.paper', overflowX: 'auto', flexShrink: 0 }}>
            ...
            <TextField variant="standard" value={selected.title} onChange={(event) => updateMeta({ title: event.target.value })} disabled={!!selected.deletedAt} fullWidth inputProps={{ 'aria-label': '노트 제목' }} InputProps={{ disableUnderline: true, sx: { minWidth: 140, fontWeight: 850, fontSize: 16 } }} />
            {selected.type === 'code' && <Select size="small" value={selected.language || 'plaintext'} onChange={(event) => updateMeta({ language: event.target.value })} sx={{ minWidth: 104, height: 30, flexShrink: 0 }}>{languageOptions.map((language) => <MenuItem key={language} value={language}>{language}</MenuItem>)}</Select>}
            ...
            {selected.type === 'code' && codeSession && !codePanelOpen && <Tooltip title="실행 콘솔 보기"><IconButton size="small" aria-label="실행 콘솔 보기" onClick={() => { setTerminalOpen(false); setCodePanelOpen(true); }}><TerminalIcon fontSize="small" /></IconButton></Tooltip>}
            ...
            {!selected.deletedAt && <Tooltip title="현재 노트북 경로에서 격리 터미널 열기"><span><IconButton size="small" aria-label="터미널 열기" disabled={!selected.notebookId} onClick={() => openNotebookTerminal(selected.notebookId)}><TerminalIcon fontSize="small" /></IconButton></span></Tooltip>}
            {!selected.deletedAt && <Tooltip title="연결 문서 만들기"><span><Button size="small" variant="outlined" startIcon={<DescriptionIcon />} disabled={officeCreating || savingState === 'dirty' || savingState === 'saving'} onClick={(event) => setOfficeMenu({ anchorEl: event.currentTarget })}>문서</Button></span></Tooltip>}
            {!selected.deletedAt && <Tooltip title="버전 기록"><IconButton size="small" aria-label="버전 기록" onClick={openVersions}><HistoryIcon fontSize="small" /></IconButton></Tooltip>}
            {!selected.deletedAt && <Tooltip title="파일로 내보내기"><IconButton size="small" aria-label="파일로 내보내기" onClick={exportSelected}><DownloadIcon fontSize="small" /></IconButton></Tooltip>}
            {!selected.deletedAt && <Tooltip title={savingState === 'dirty' || savingState === 'saving' ? '저장이 끝난 뒤 첨부할 수 있습니다.' : 'NAS 파일 또는 폴더 첨부'}><span><IconButton size="small" aria-label="NAS 파일 또는 폴더 첨부" disabled={savingState === 'dirty' || savingState === 'saving'} onClick={() => setAttachmentPickerOpen(true)}><AttachFileIcon fontSize="small" /></IconButton></span></Tooltip>}
            {!selected.deletedAt ? <Tooltip title="휴지통으로 이동"><IconButton size="small" aria-label="휴지통으로 이동" color="error" onClick={trashSelected}><DeleteOutlineIcon fontSize="small" /></IconButton></Tooltip> : <><Tooltip title="복원"><IconButton size="small" aria-label="복원" color="primary" onClick={restoreSelected}><RestoreIcon fontSize="small" /></IconButton></Tooltip><Tooltip title="영구 삭제"><IconButton size="small" aria-label="영구 삭제" color="error" onClick={permanentlyDelete}><DeleteForeverIcon fontSize="small" /></IconButton></Tooltip></>}
          </Stack>
