...
const Settings = () => {
  ...
  const handleReject = (id) => axios.post('/api/users/reject', { id }, { withCredentials: true }).then(() => setPendingUsers(prev => prev.filter(p => p.id !== id)));
                ...
                <TableRow key={u.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                  ...
                  <TableCell>
                    <input 
                      ...
                      value={u.rootPath || u.root_path || ''} 
                      onChange={(e) => setUsers(users.map(user => user.id === u.id ? { ...user, rootPath: e.target.value } : user))} 
                      style={{ padding: '5px', width: '100%', borderRadius: '4px', border: '1px solid #ccc', boxSizing: 'border-box' }} 
                      disabled={!isMaster && u.role === 'MASTER'} 
                    />
                  </TableCell>
                  <TableCell align="center">
