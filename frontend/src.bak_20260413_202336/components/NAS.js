...
const NAS = () => {
  ...
  const saveFile = async (win) => {
    try { const blob = new Blob([win.content], { type: 'text/plain' }); const file = new File([blob], win.name, { type: 'text/plain' }); const formData = new FormData(); formData.append('path', ensureSlash(win.fullPath.substring(0, win.fullPath.lastIndexOf('/')))); formData.append('file', file);
      await axios.post('/api/file', formData, { withCredentials: true, headers: { 'Content-Type': 'multipart/form-data' } }); setOpenWindows(prev => prev.map(w => w.id === win.id ? { ...w, originalContent: win.content, mode: 'view' } : w)); setSnackbar({ open: true, message: `'${win.name}' 저장 완료.`, severity: 'success' }); } catch (err) { showError('저장', err); }
  };
  ...
  const handleUploadClick = (targetPath) => { uploadTargetRef.current = ensureSlash(targetPath); if(fileInputRef.current) { fileInputRef.current.value = ''; fileInputRef.current.click(); } };

  const handleFileUpload = async (e) => {
    const file = e.target.files[0]; if (!file) return; 
    const targetPath = ensureSlash(uploadTargetRef.current); setSnackbar({ open: true, message: `'${file.name}' 업로드 중...`, severity: 'info' });
    const formData = new FormData(); formData.append('path', targetPath); formData.append('file', file);
    try { await axios.post('/api/file', formData, { withCredentials: true, headers: { 'Content-Type': 'multipart/form-data' }, onUploadProgress: (evt) => { const p = Math.round((evt.loaded * 100) / evt.total); setSnackbar({ open: true, message: p === 100 ? `서버 저장 중...` : `전송 중... ${p}%`, severity: 'info' }); } }); 
      setSnackbar({ open: true, message: `'${file.name}' 완료!`, severity: 'success' }); refreshPath(targetPath); } catch (err) { showError('업로드', err); } finally { if (fileInputRef.current) fileInputRef.current.value = ''; }
  };
  ...
  const handleDragStart = (e, item, sourceId) => { const rect = e.target.getBoundingClientRect(); const safePath = ensureSlash(item.fullPath); let currentSelection = selectedItems.includes(safePath) ? selectedItems : [safePath]; if (!selectedItems.includes(safePath)) setSelectedItems(currentSelection); e.dataTransfer.setData('application/json', JSON.stringify({ draggedPaths: currentSelection, anchorPath: safePath, sourceId, offsetX: e.clientX - rect.left, offsetY: e.clientY - rect.top })); e.dataTransfer.effectAllowed = 'move'; };
    ...
    <Box sx={{ flex: 1, position: 'relative', display: 'flex', flexDirection: 'column', overflow: 'hidden', height: '100%', userSelect: 'none', '& .react-resizable-handle:hover': { backgroundColor: theme.palette.primary.main, opacity: 0.5 }}}>
      <Box ref={desktopRef} onDragOver={(e) => handleDragOver(e, null)} onDrop={(e) => handleDrop(e, '/', 'desktop')} onContextMenu={(e) => handleContextMenu(e, 'background', { path: '/', windowId: 'desktop' })} onMouseDown={(e) => { if (isLongPressTriggered.current) return; if (e.target === e.currentTarget) { setFocusedContext('desktop'); setSelectedItems([]); setInlineEdit(null); } }} onTouchStart={(e) => { if (e.target === e.currentTarget) handleTouchStart(e, 'background', { path: '/', windowId: 'desktop' }); }} onTouchMove={cancelTouch} onTouchEnd={cancelTouch} onTouchCancel={cancelTouch} sx={{ flex: 1, position: 'relative', overflowX: 'hidden', overflowY: 'auto', display: isMobile ? 'flex' : 'block', flexWrap: isMobile ? 'wrap' : 'nowrap', alignContent: 'flex-start', gap: isMobile ? 2 : 0, p: isMobile ? 2 : 0, background: theme.palette.mode === 'dark' ? 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)' : 'linear-gradient(135deg, #1e3a8a 0%, #3b82f6 50%, #93c5fd 100%)' }}>
        <motion.div onClick={(e) => { e.stopPropagation(); setSelectedItems(['system_root']); if (isMobile) openFolderWindow({ id: 'system_root', name: isAdmin ? '서버 전체 저장소' : '내 클라우드', path: '/' }); }} onDoubleClick={(e) => { e.stopPropagation(); if(!isMobile) openFolderWindow({ id: 'system_root', name: isAdmin ? '서버 전체 저장소' : '내 클라우드', path: '/' }); }} style={isMobile ? { textAlign: 'center', width: '80px', cursor: 'pointer', zIndex: 10 } : { position: 'absolute', left: 20, top: 20, textAlign: 'center', cursor: 'pointer', width: '100px', zIndex: 10 }}>
          <Box sx={{ p: 1, borderRadius: '6px', border: selectedItems.includes('system_root') ? '1px solid rgba(255,255,255,0.4)' : '1px solid transparent', backgroundColor: selectedItems.includes('system_root') ? 'rgba(255, 255, 255, 0.15)' : 'transparent', color: 'white', filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.3))' }}><StorageIcon sx={{ fontSize: isMobile ? 40 : 60, color: isAdmin ? '#ef4444' : '#fde047' }} /><Typography variant="body2" sx={{ mt: 1, fontWeight: 'bold', fontSize: isMobile ? '0.75rem' : '0.875rem' }}>{isAdmin ? '전체 저장소' : '내 클라우드'}</Typography></Box>
        </motion.div>
        ...
        {inlineEdit && (inlineEdit.mode === 'new' || inlineEdit.mode === 'newFile') && inlineEdit.windowId === 'desktop' && (<motion.div style={isMobile ? { textAlign: 'center', width: '80px', zIndex: 15 } : { position: 'absolute', left: inlineEdit.spawnPosition?.x || 200, top: inlineEdit.spawnPosition?.y || 200, textAlign: 'center', width: '100px', zIndex: 15 }}><Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.3))' }}>{inlineEdit.mode === 'new' ? <FolderIcon sx={{ fontSize: isMobile ? 40 : 60, color: '#fde047' }} /> : <InsertDriveFileIcon sx={{ fontSize: isMobile ? 40 : 60, color: '#e2e8f0' }} />}<InlineInput defaultValue={inlineEdit.name} isDesktop={true} onSubmit={(val) => handleInlineSubmit(val, inlineEdit)} onCancel={() => setInlineEdit(null)} /></Box></motion.div>)}
      </Box>
